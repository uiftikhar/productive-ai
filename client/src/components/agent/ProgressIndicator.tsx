...
export default function ProgressIndicator({
  ...
  const { progress, status, isLoading, error, details } = useAgentProgress(sessionId, {
    pollingInterval,
    ...
    <div className={`rounded border p-3 ${className}`}>
      <div className='mb-2 flex items-center justify-between'>
        <div className='flex items-center'>
          ...
          <span className={`text-sm font-medium ${getStatusColor()}`}>{getStatusDisplay()}</span>
        </div>
      ...
      {showDetails && details && (
        <div className='mt-2 text-xs text-gray-500'>
          ...
          {details.updatedAt && <p>Updated: {new Date(details.updatedAt).toLocaleString()}</p>}
          {details.completedAt && (
