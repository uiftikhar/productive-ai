          ...
          <AlertDescription>
            <ul className="list-disc pl-5 mt-2">
              {resultData.errors.map((error, i) => (
                <li key={i}>{error.step}: {error.error}</li>
              ))}
            </ul>
          </AlertDescription>
        </Alert>
        ...
        <TabsContent value="topics">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                {resultData?.topics ? `${resultData.topics.length} topics identified` : 'No topics identified yet'}
              </CardDescription>
            </CardHeader>
            <CardContent>
                            ...
                            <div className="mt-3">
                              ...
                              <div className="flex flex-wrap gap-1 mt-1">
                                {topic.keywords.map((keyword: string, j: number) => (
                                  <Badge variant="outline" key={j}>{keyword}</Badge>
                                ))}
                              </div>
                            </div>
                          )}
        ...
        <TabsContent value="action-items">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                {resultData?.actionItems ? `${resultData.actionItems.length} action items identified` : 'No action items identified yet'}
              </CardDescription>
            </CardHeader>
            <CardContent>
                ...
                <div className="space-y-6">
                  <div className="flex justify-between items-center">
                    ...
                    <Badge className={`px-3 py-1 ${
                      (resultData.sentiment?.overall || resultData.sentiment?.overallSentiment) === 'positive' ? 'bg-green-100 text-green-700' :
                      (resultData.sentiment?.overall || resultData.sentiment?.overallSentiment) === 'negative' ? 'bg-red-100 text-red-700' :
                      (resultData.sentiment?.overall || resultData.sentiment?.overallSentiment) === 'mixed' ? 'bg-yellow-100 text-yellow-700' :
                      'bg-gray-100 text-gray-700'
                    }`}>
                      {((resultData.sentiment?.overall || resultData.sentiment?.overallSentiment) || 'neutral').charAt(0).toUpperCase() + 
                      ((resultData.sentiment?.overall || resultData.sentiment?.overallSentiment) || 'neutral').slice(1)}
                      {(resultData.sentiment?.score !== undefined || resultData.sentiment?.sentimentScore !== undefined) ? 
                        ` (${((resultData.sentiment?.score ?? resultData.sentiment?.sentimentScore) ?? 0).toFixed(2)})` : ''}
                    </Badge>
                  </div>
                  
                  {resultData.sentiment?.keyEmotions && resultData.sentiment.keyEmotions.length > 0 && (
                    <div>
                      <h3 className="text-lg font-medium mb-2">Key Emotions</h3>
                  ...
                  {resultData.sentiment?.topicSentiments && resultData.sentiment.topicSentiments.length > 0 && (
                    <div>
                      <h3 className="text-lg font-medium mb-2">Topic Sentiments</h3>
                      <ScrollArea className="h-[200px] pr-4">
                        <div className="space-y-3">
                          {resultData.sentiment.topicSentiments.map((topicSentiment, i) => (
                            <div key={i} className={`p-3 rounded-lg ${
                              topicSentiment.sentiment === 'positive' ? 'bg-green-50 border-l-4 border-green-400' :
                              topicSentiment.sentiment === 'negative' ? 'bg-red-50 border-l-4 border-red-400' :
                              'bg-gray-50 border-l-4 border-gray-400'
                            }`}>
                              <div className="flex justify-between mb-1">
                                <span className="text-sm font-medium">{topicSentiment.topic}</span>
                                <span className="text-xs">
                                  {topicSentiment.sentiment.charAt(0).toUpperCase() + topicSentiment.sentiment.slice(1)} 
                  ...
                  {resultData.sentiment?.segments && resultData.sentiment.segments.length > 0 && (
                    <div>
                      <h3 className="text-lg font-medium mb-2">Sentiment Segments</h3>
                      <ScrollArea className="h-[300px] pr-4">
                        <div className="space-y-3">
                          {resultData.sentiment.segments.map((segment, i) => (
                            <div key={i} className={`p-3 rounded-lg ${
                              segment.sentiment === 'positive' ? 'bg-green-50 border-l-4 border-green-400' :
                              segment.sentiment === 'negative' ? 'bg-red-50 border-l-4 border-red-400' :
                              'bg-gray-50 border-l-4 border-gray-400'
                            }`}>
                              <div className="flex justify-between mb-1">
