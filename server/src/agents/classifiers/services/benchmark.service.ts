...
export class BenchmarkService {
  ...
  async runBenchmark(
    ...
    const mockAgents = Object.values(agents).reduce(
      (acc, agent) => {
        acc[agent.id] = {
          id: agent.id,
          name: agent.name,
    ...
    for (const item of dataset) {
      try {
        ...
        if (options.verbose) {
          this.logger.info(
            `Test case: ${item.category} - ${isCorrect ? '✓' : '✗'}`,
          );
          this.logger.info(`  Input: ${item.input.substring(0, 50)}...`);
          this.logger.info(
            `  Expected: ${item.expected.agentId}, Got: ${result.selectedAgentId}`,
          );
          const reasoning =
    ...
    const accuracy =
      dataset.length > 0 ? correctClassifications / dataset.length : 0;

    if (options.verbose) {
      this.logger.info(`Benchmark completed in ${endTime - startTime}ms`);
      this.logger.info(
        `Accuracy: ${(accuracy * 100).toFixed(2)}% (${correctClassifications}/${dataset.length})`,
      );
    }
  ...
  logResults(results: BenchmarkStats, verbose = false): void {
    ...
    this.logger.info(
      `Failed: ${results.totalTests - results.correctClassifications}`,
    );
    ...
    if (failedTests.length > 0 && verbose) {
      ...
      failedTests.forEach((detail) => {
        this.logger.info(
          `- Category: ${detail.item.category}, Input: "${detail.item.input}"`,
        );
