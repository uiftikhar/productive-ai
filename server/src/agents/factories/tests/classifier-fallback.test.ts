...
const createMockAgent = (id: string, name: string): MockProxy<BaseAgentInterface> => {
  ...
  Object.defineProperty(mockAgent, 'description', { value: `Mock agent (${name})` });
...
interface DefaultAgentInterface {
  processFallbackLogic: (result: ClassifierResult, input: string) => ClassifierResult;
...
describe('Classifier Fallback Integration', () => {
  ...
  beforeEach(() => {
    ...
    mockSpecialistAgent = createMockAgent('specialist-agent', 'Specialist Agent');
    ...
    mockDefaultAgentService.processFallbackLogic.mockImplementation((result, input) => {
  ...
  describe('Advanced Fallback Integration', () => {
    it('should track fallback metrics correctly', async () => {
      ...
      await classifierFactory.classify('normal request', [], { enableDefaultAgentFallback: true });
      await classifierFactory.classify('unknown topic', [], { enableDefaultAgentFallback: true });
      await classifierFactory.classify('unsure about this', [], { enableDefaultAgentFallback: true });
    ...
    it('should support context-aware fallback decisions', async () => {
      ...
      const result = await classifierFactory.classify('unsure about this follow-up', history, {
        enableDefaultAgentFallback: true
      });
  ...
  describe('Threshold Tuning', () => {
    it('should adjust thresholds based on metrics', () => {
      ...
      expect(mockDefaultAgentService.setConfidenceThreshold).toHaveBeenCalledWith(
        expect.any(Number)
      );
    });
  });
