...
export class ConsensusVotingService {
  ...
  createVotingSession(
    ...
    if (!options.title || !options.options || options.options.length < 2) {
      throw new Error('Invalid voting session: requires title and at least 2 options');
    }
  ...
  private async notifyEligibleVoters(session: VotingSession): Promise<void> {
    for (const voterId of session.eligibleVoters) {
      ...
      try {
        ...
        const canVote = capabilities.some(cap => 
          cap.name === 'voting' || cap.name === 'consensus-voting'
        );
  ...
  castVote(vote: Omit<Vote, 'timestamp'>): Vote {
    ...
    if (!session.eligibleVoters.includes(vote.voterId)) {
      throw new Error(`Voter ${vote.voterId} is not eligible for session ${vote.sessionId}`);
    }
  ...
  private checkConsensus(sessionId: string): boolean {
    ...
    const threshold = session.metadata?.consensusThreshold || this.defaultConsensusThreshold;
    ...
    for (const [sessionId, session] of this.votingSessions.entries()) {
      if (session.status === 'open') {
        try {
          ...
          this.logger.warn(`Error closing session ${sessionId} during cleanup`, { error });
        }
      }
    }
  }
