...
export class TaskBreakdownService {
  ...
  public async initiateTaskBreakdown(
    ...
    this.logger.info(
      `Initiating collaborative breakdown for task ${taskId} by agent ${proposingAgentId}`,
    );
   ...
   */
  public addCollaborator(breakdownId: string, collaboratorId: string): boolean {
    ...
    if (breakdown.collaborators.includes(collaboratorId)) {
      this.logger.info(
        `Agent ${collaboratorId} is already a collaborator on breakdown ${breakdownId}`,
      );
      return true;
    }
    ...
    this.logger.info(
      `Added agent ${collaboratorId} as collaborator on breakdown ${breakdownId}`,
    );
    return true;
  }
  ...
  public submitBreakdownVote(
    ...
    if (!breakdown.collaborators.includes(agentId)) {
      this.logger.warn(
        `Agent ${agentId} is not a collaborator on breakdown ${breakdownId}`,
      );
      return false;
    }

    // Check if already voted
    const existingVoteIndex = breakdown.votes.findIndex(
      (v) => v.agentId === agentId,
    );
   ...
   */
  private checkBreakdownVotingStatus(
    breakdown: CollaborativeTaskBreakdown,
  ): void {
    ...
    const approveVotes = breakdown.votes.filter(
      (v) => v.vote === 'approve',
    ).length;
    const rejectVotes = breakdown.votes.filter(
      (v) => v.vote === 'reject',
    ).length;
    ...
    if (approveVotes > totalVotes / 2) {
      ...
      this.logger.info(
        `Breakdown ${breakdown.breakdownId} approved with ${approveVotes}/${totalVotes} votes`,
      );
      ...
      this.logger.info(
        `Breakdown ${breakdown.breakdownId} rejected with ${rejectVotes}/${totalVotes} votes`,
      );
    }
  }
  ...
  private calculateBreakdownMetrics(
    ...
    const avgComplexity =
      subtasks.reduce((sum, task) => sum + task.estimatedComplexity, 0) /
      subtasks.length;
    ...
    subtasks.forEach((task) => {
      if (task.suggestedAgentId && task.requiredCapabilities) {
        ...
        if (agent) {
          ...
          const matchedCapabilities = task.requiredCapabilities.filter((rc) =>
  ...
  public updateSubtasks(
    ...
    if (breakdown.status !== 'draft' && breakdown.status !== 'rejected') {
      this.logger.warn(
        `Cannot update breakdown ${breakdownId} in ${breakdown.status} status`,
      );
      return false;
    }
  ...
  public startVoting(breakdownId: string, agentId: string): boolean {
    ...
    if (
      breakdown.proposingAgentId !== agentId &&
      !breakdown.collaborators.includes(agentId)
    ) {
      this.logger.warn(
        `Agent ${agentId} cannot start voting for breakdown ${breakdownId}`,
      );
      return false;
    }
    ...
    if (breakdown.status !== 'draft') {
      this.logger.warn(
        `Cannot start voting for breakdown ${breakdownId} in ${breakdown.status} status`,
      );
      return false;
    }
  ...
  public getBreakdown(
    breakdownId: string,
  ): CollaborativeTaskBreakdown | undefined {
  ...
  public getBreakdownsForTask(taskId: string): CollaborativeTaskBreakdown[] {
    return Array.from(this.breakdowns.values()).filter(
      (breakdown) => breakdown.taskId === taskId,
    );
  }
}
