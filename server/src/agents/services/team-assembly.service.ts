...
export class TeamAssemblyService {
  ...
  private agentPerformanceCache: Map<
    string,
    {
      successRate: number;
  ...
  public async formTaskTeam(
    ...
    this.logger.info(
      `Forming team for task ${taskId} using strategy: ${strategy}`,
    );
      ...
      .map((agent) => {
        ...
        switch (strategy) {
          ...
          case 'balanced':
            ...
            strategyScore =
              capabilityScore * 0.5 + (agentCapabilities.length / 10) * 0.5;
            break;
        }

        // Calculate final score
        const finalScore =
          capabilityScore * 0.5 + performanceScore * 0.3 + strategyScore * 0.2;
        ...
        if (
          ...
          agentCapabilities.includes('task-planning') ||
    ...
    while (
      ...
      teamMembers.length < scoredAgents.length
    ) {
      ...
      for (const memberId of selectedMembers) {
        ...
        if (member) {
          member.agentCapabilities.forEach((cap) =>
      ...
      for (const agent of scoredAgents) {
        ...
        const roleBonus = !teamMembers.some((m) => m.role === agent.role)
          ? 0.2
          : 0;
    ...
    this.logger.info(
      `Formed team with ${teamMembers.length} members for task ${taskId}`,
    );
  ...
  public analyzeCapabilityComposition(
    ...
    const specializationSum = analyzedCapabilities.reduce(
      (sum, cap) => sum + cap.specializationLevel,
      0,
    );

    const recommendedTeamSize = Math.max(
      2,
      Math.min(5, Math.ceil(analyzedCapabilities.length / 2)),
    );
