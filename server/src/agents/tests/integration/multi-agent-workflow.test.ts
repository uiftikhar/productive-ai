...
interface TestAgentFactory extends AgentFactory {
  createDocumentRetrievalAgent(
    options?: any,
  ): DocumentRetrievalAgent | AgentWorkflow<any>;
  createKnowledgeRetrievalAgent(
    options?: any,
  ): KnowledgeRetrievalAgent | AgentWorkflow<any>;
  createDecisionTrackingAgent(
    options?: any,
  ): DecisionTrackingAgent | AgentWorkflow<any>;
  createMeetingAnalysisAgent(
    options?: any,
  ): MeetingAnalysisAgent | AgentWorkflow<any>;
...
describe('Multi-Agent Workflow Integration Tests', () => {
  ...
  beforeEach(() => {
    ...
    jest
      .spyOn(retrievalAgent, 'executeInternal')
    ...
    jest
      .spyOn(knowledgeAgent, 'executeInternal')
      .mockImplementation(async (request) => {
        // Use any assertion to bypass type checking for tests
        const contextData = request.context as any;
        const documentInfo = contextData?.document
          ? `based on ${contextData.document.id}`
          : '';
    ...
    jest
      .spyOn(decisionAgent, 'executeInternal')
      .mockImplementation(async (request) => {
        // Use any assertion to bypass type checking for tests
        const contextData = request.context as any;
        ...
        return {
          ...
          artifacts: {
            ...
            confidence: 0.85,
            justification:
              'Based on knowledge insights and retrieved documents',
          },
        };
      });

    jest
      .spyOn(meetingAnalysisAgent, 'executeInternal')
      .mockImplementation(async (request) => {
        return {
  ...
  test('should execute multiple agents in sequence with context passing', async () => {
    ...
    const retrievalWorkflow = new AgentWorkflow(retrievalAgent, {
      tracingEnabled: true,
    });
    const knowledgeWorkflow = new AgentWorkflow(knowledgeAgent, {
      tracingEnabled: true,
    });
    const decisionWorkflow = new AgentWorkflow(decisionAgent, {
      tracingEnabled: true,
    });
    ...
    expect((retrievalWorkflow as any).getLastState().status).toBe(
      WorkflowStatus.READY,
    );
    expect((knowledgeWorkflow as any).getLastState().status).toBe(
      WorkflowStatus.READY,
    );
    expect((decisionWorkflow as any).getLastState().status).toBe(
      WorkflowStatus.READY,
    );
  });
  ...
  test('should select appropriate agents based on capabilities', async () => {
    ...
    const discoveredRetrievalAgent = registry.getAgent(
      retrievalDiscovery!.agentId,
    );
    expect(discoveredRetrievalAgent).toBeDefined();

    // Cast to any to bypass type checking for test mocks
    const retrievalWorkflow = new AgentWorkflow(
      discoveredRetrievalAgent as BaseAgent,
    );
    const retrievalResult = await retrievalWorkflow.execute({
      input: query,
      capability: 'searchDocuments',
    ...
    const discoveredKnowledgeAgent = registry.getAgent(
      knowledgeDiscovery2!.agentId,
    );
    expect(discoveredKnowledgeAgent).toBeDefined();

    // Cast to any to bypass type checking for test mocks
    const knowledgeWorkflow = new AgentWorkflow(
      discoveredKnowledgeAgent as BaseAgent,
    );
    const knowledgeResult = await knowledgeWorkflow.execute({
      input: `Analyze ${query}`,
      capability: 'retrieve_knowledge',
  ...
  test('should execute a complex research workflow with orchestration', async () => {
    ...
    const coordinatorWorkflow = new AgentWorkflow(meetingAnalysisAgent, {
      tracingEnabled: true,
    });
    ...
    expect((coordinatorWorkflow as any).getLastState().status).toBe(
      WorkflowStatus.READY,
    );
  });
});
