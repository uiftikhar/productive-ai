...
describe('Chat API Integration Tests', () => {
  ...
  describe('POST /api/chat/sessions', () => {
    it('should create a new session', async () => {
      ...
      expect(response.body.session.conversationId).toBe(
        mockSession.conversationId,
      );
  ...
  describe('GET /api/chat/sessions/:sessionId', () => {
    ...
    it('should return 404 when session is not found', async () => {
      ...
      // Test request
      const response = await request(app).get(
        '/api/chat/sessions/non-existent-session',
      );
  ...
  describe('POST /api/chat/messages', () => {
    it('should send a message and get a response', async () => {
      ...
      expect(response.body.result.message.content).toBe(
        mockResult.message.content,
      );
      expect(response.body.result.message.role).toBe(mockResult.message.role);
      expect(response.body.result.message.sessionId).toBe(
        mockResult.message.sessionId,
      );
      expect(response.body.result.agentsInvolved).toEqual(
        mockResult.agentsInvolved,
      );
      expect(response.body.result.primaryAgent).toBe(mockResult.primaryAgent);
      expect(response.body.result.executionTimeMs).toBe(
        mockResult.executionTimeMs,
      );
  ...
  describe('GET /api/chat/users/:userId/sessions', () => {
    it('should get sessions for a user', async () => {
      ...
      // Test request
      const response = await request(app).get(
        '/api/chat/users/test-user-123/sessions',
      );
      ...
      expect(response.body.sessions[0].sessionId).toBe(
        mockSessions[0].sessionId,
      );
      expect(response.body.sessions[0].userId).toBe(mockSessions[0].userId);
      expect(response.body.sessions[0].conversationId).toBe(
        mockSessions[0].conversationId,
      );
      expect(response.body.sessions[1].sessionId).toBe(
        mockSessions[1].sessionId,
      );
      expect(response.body.sessions[1].userId).toBe(mockSessions[1].userId);
      expect(response.body.sessions[1].conversationId).toBe(
        mockSessions[1].conversationId,
      );

      expect(mockChatService.getUserSessions).toHaveBeenCalledWith(
        'test-user-123',
      );
    });
  });
