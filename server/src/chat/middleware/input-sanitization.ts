...
export function createSanitizationMiddleware(options: SanitizationOptions = {}) {
  ...
  const xssOptions = {
    whiteList: options.allowedTags ? {} as Record<string, string[]> : undefined,
        ...
        for (const header of sanitizeHeaders) {
          ...
          if (headerValue) {
            if (typeof headerValue === 'string') {
              req.headers[header.toLowerCase()] = xssFilter.process(headerValue);
            } else if (Array.isArray(headerValue)) {
              req.headers[header.toLowerCase()] = headerValue.map(v => xssFilter.process(v));
            }
          }
        }
      }
