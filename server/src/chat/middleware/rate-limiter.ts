...
export function createRateLimiter(options: RateLimitOptions) {
  ...
  ).unref(); // Clean up at least every minute
  ...
  return function rateLimiter(req: Request, res: Response, next: NextFunction) {
    ...
    if (useSliding) {
      ...
      const currentBucketTime =
        Math.floor(now / bucketInterval) * bucketInterval;
      let currentBucket = record.buckets.find(
        (b) => b.timestamp === currentBucketTime,
      );
    ...
    const exceeded = useSliding ? remaining <= 0 : record.count > limit;
