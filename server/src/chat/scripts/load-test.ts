...
const scenarioConfig =
  scenarios[config.scenario as keyof typeof scenarios] || scenarios.normal;
  ...
  generateRandomMessage(): string {
    ...
    if (Math.random() < scenarioConfig.errorRate) {
      ...
      if (errorType === 'invalid_chars') {
        ...
        message +=
          ' ' +
          specialStrings[Math.floor(Math.random() * specialStrings.length)];
      }
    }
...
async function runLoadTest() {
  console.log(
    `Starting load test with ${config.users} users, ${config.duration}s duration`,
  );
  console.log(
    `Scenario: ${config.scenario}, Message rate: ${config.messagesPerSecond}/s per user`,
  );
  ...
  console.log(
    `All users connected. Test running for ${config.duration} seconds...`,
  );
  ...
  const metricsInterval = setInterval(() => {
    ...
    console.log(
      `[${elapsedSeconds.toFixed(1)}s] Requests: ${metrics.totalRequests}, ` +
  ...
  console.log(
    `Requests/second: ${(metrics.totalRequests / (testDurationMs / 1000)).toFixed(2)}`,
  );
