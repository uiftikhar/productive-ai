...
export class TranscriptAnalysisController {
  ...
  private constructor() {
    ...
    this.agenticSystem.initialize()
      ...
      .catch((error: unknown) => {
        ...
        this.logger.error('Failed to initialize Agentic Meeting Analysis System', { error: errorMessage });
      });
  }
  ...
  public async analyzeTranscript(req: Request, res: Response): Promise<void> {
    try {
      ...
      this.logger.error('Error analyzing transcript', { error: errorObj.message });
      res.status(500).json({ 
  ...
  public async getAnalysisStatus(req: Request, res: Response): Promise<void> {
    try {
      ...
      this.logger.error('Error getting analysis status', { error: errorObj.message });
      res.status(500).json({ 
