 ...
 */
export class CapabilityAllocationServiceImpl implements ICapabilityAllocationService {
  ...
  allocateResources(request: ResourceAllocationRequest): ResourceAllocation {
    this.logger.info(`Resource allocation requested for task ${request.taskId}`, {
      taskId: request.taskId,
      requiredCapabilities: request.requiredCapabilities.map(c => c.capabilityId),
    ...
    for (const capability of request.requiredCapabilities) {
      ...
      if (request.excludedResources && request.excludedResources.length > 0) {
        resources = resources.filter(r => !request.excludedResources!.includes(r.id));
      }
      ...
      if (capability.preferredLevel) {
        resources.sort((a, b) => {
          const aCapability = a.capabilities.find(c => c.id === capability.capabilityId);
          const bCapability = b.capabilities.find(c => c.id === capability.capabilityId);
      ...
      if (resources.length > 0) {
        ...
        capabilityAllocation.get(bestResource.id)?.push(capability.capabilityId);
        ...
        if (capabilityResource) {
          const levelQuality = capability.preferredLevel
            ? this.calculateLevelMatchQuality(capabilityResource.level, capability.preferredLevel)
            : 1.0;
        ...
        if (capability.essential) {
          this.logger.warn(`Essential capability ${capability.capabilityId} could not be allocated for task ${request.taskId}`);
        }
      }
    }

    // Check if all essential capabilities were allocated
    const failedEssentials = essentialCapabilities.filter(cap => unallocatedCapabilities.includes(cap));
    
    if (failedEssentials.length > 0) {
      this.logger.error(`Failed to allocate essential capabilities for task ${request.taskId}`, {
        taskId: request.taskId,
    ...
    const overallQuality = allocatedCapabilities.length > 0
    ...
    const allocated = Array.from(capabilityAllocation.entries()).map(([resourceId, capabilities]) => ({
      resourceId,
      capabilities,
    ...
    this.logger.info(`Resource allocation completed for task ${request.taskId}`, {
      taskId: request.taskId,
  ...
  updateAllocation(taskId: string, updatedRequest: Partial<ResourceAllocationRequest>): ResourceAllocation {
    ...
    if (!currentAllocation) {
      ...
      return this.allocateResources(updatedRequest as ResourceAllocationRequest);
    }
  ...
  private findOriginalRequest(taskId: string): ResourceAllocationRequest {
    ...
    defaultRequest.requiredCapabilities = Array.from(capabilities).map(capId => ({
      capabilityId: capId,
  ...
  getResourceAllocations(resourceId: string): ResourceAllocation[] {
    ...
    return Array.from(taskIds)
      ...
      .filter((allocation): allocation is ResourceAllocation => allocation !== undefined);
  }
  ...
  findBestResourceForCapability(capabilityId: string, level?: CapabilityLevel): string {
    ...
    const resources = this.availabilityService.getResourcesByCapability(capabilityId, level);
    ...
    const scoredResources = resources.map(resource => {
      // Get the capability
      const capability = resource.capabilities.find(c => c.id === capabilityId);
      if (!capability) {
      ...
      const performanceScore = capability.performanceMetrics?.successRate || 0.5;
   ...
   */
  private calculateLevelMatchQuality(actual: CapabilityLevel, preferred: CapabilityLevel): number {
    const levels = Object.values(CapabilityLevel);
  ...
  getMetrics(): Record<string, any> {
    ...
    const successfulAllocations = this.allocationHistory.filter(
      allocation => allocation.allocationQuality >= this.allocationScoreThreshold
    ).length;
    ...
    const resourceUtilization = this.availabilityService.getResourceUtilization();
