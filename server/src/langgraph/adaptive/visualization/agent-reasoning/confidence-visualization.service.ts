...
export class ConfidenceVisualizationImpl implements ConfidenceVisualization {
  ...
  private confidenceRecords: Map<
    string,
    Map<string, { timestamp: Date; value: number }[]>
  > = new Map();
  ...
  constructor(options: {
    ...
    if (!options.reasoningPathService) {
      throw new Error(
        'Confidence visualization service requires a reasoning path service',
      );
    }
  ...
  recordConfidenceLevel(
    ...
    try {
      ...
      this.logger.debug(
        `Recorded confidence level ${confidence} for agent ${agentId} on path ${pathId}`,
      );
  ...
  visualizeConfidenceOverTime(pathId: string): any {
    try {
      ...
      for (let i = 0; i < confidenceRecords.length; i++) {
        ...
        const average =
          windowRecords.reduce((sum, record) => sum + record.value, 0) /
      ...
      if (confidenceRecords.length >= 2) {
        const firstHalf = confidenceRecords.slice(
          0,
          Math.floor(confidenceRecords.length / 2),
        );
        const secondHalf = confidenceRecords.slice(
          Math.floor(confidenceRecords.length / 2),
        );

        const firstAvg =
          firstHalf.reduce((sum, record) => sum + record.value, 0) /
          firstHalf.length;
        const secondAvg =
          secondHalf.reduce((sum, record) => sum + record.value, 0) /
          secondHalf.length;
      ...
      const overallAverage =
        confidenceRecords.length > 0
          ? confidenceRecords.reduce((sum, record) => sum + record.value, 0) /
      ...
      return {
        ...
        timeRange: {
          start:
            confidenceRecords.length > 0
              ? confidenceRecords[0].timestamp
              : null,
          end:
            confidenceRecords.length > 0
              ? confidenceRecords[confidenceRecords.length - 1].timestamp
              : null,
        },
      };
    } catch (error) {
      this.logger.error(
        `Error visualizing confidence over time for path ${pathId}:`,
        { error },
      );
  ...
  getConfidenceMetrics(agentId: string): {
    ...
    try {
      ...
      const average =
        allRecords.length > 0
          ? allRecords.reduce((sum, record) => sum + record.value, 0) /
            allRecords.length
          : 0;
      ...
      if (allRecords.length >= 2) {
        const firstHalf = allRecords.slice(
          0,
          Math.floor(allRecords.length / 2),
        );
      ...
      this.logger.error(
        `Error getting confidence metrics for agent ${agentId}:`,
        { error },
      );
  ...
  compareConfidenceLevels(agentIds: string[]): any {
    try {
      ...
      const lowestAverage =
        agentComparisons[agentComparisons.length - 1].average;

      const normalizedComparisons = agentComparisons.map((agent) => ({
        ...agent,
        relativeConfidence:
          highestAverage > 0 ? agent.average / highestAverage : 0,
      }));

      // Find most stable agent (lowest volatility)
      const mostStableAgent = [...agentComparisons].sort(
  ...
  generateConfidenceTimeline(
    ...
    try {
      ...
      this.logger.error(
        `Error generating confidence timeline for agent ${agentId}:`,
        { error },
      );
