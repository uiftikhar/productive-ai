...
export class ReasoningPathImpl implements ReasoningPathVisualization {
  ...
  constructor(options: {
    ...
    if (!options.decisionCapture) {
      throw new Error(
        'Reasoning path service requires a decision capture service',
      );
    }
  ...
  createReasoningPath(agentId: string, taskId?: string): string {
    ...
    this.logger.info(
      `Created reasoning path ${pathId} for agent ${agentId}${taskId ? ` and task ${taskId}` : ''}`,
    );
  ...
  addDecisionToPath(pathId: string, decisionId: string): boolean {
    ...
    try {
      ...
      if (decision.agentId !== path.agentId) {
        this.logger.warn(
          `Cannot add decision ${decisionId} from agent ${decision.agentId} to path ${pathId} for agent ${path.agentId}`,
        );
        return false;
      }
      ...
      if (path.taskId && decision.taskId && path.taskId !== decision.taskId) {
        this.logger.warn(
          `Cannot add decision ${decisionId} for task ${decision.taskId} to path ${pathId} for task ${path.taskId}`,
        );
        return false;
      }
      ...
      this.logger.info(
        `Added decision ${decisionId} to reasoning path ${pathId}`,
      );

      return true;
    } catch (error) {
      this.logger.error(
        `Error adding decision ${decisionId} to path ${pathId}:`,
        { error },
      );
      return false;
    }
  ...
  visualizeReasoningPath(pathId: string): any {
    ...
    const avgConfidence =
      path.confidenceOverTime.length > 0
        ? path.confidenceOverTime.reduce((sum, point) => sum + point.value, 0) /
          path.confidenceOverTime.length
        : 0;
  ...
  compareReasoningPaths(path1Id: string, path2Id: string): any {
    ...
    const timingComparison = {
      ...
      durationDifference:
        viz1.duration !== null && viz2.duration !== null
    ...
    const minLength = Math.min(
      path1.decisionPoints.length,
      path2.decisionPoints.length,
    );
    ...
    for (let i = 0; i < minLength; i++) {
      ...
      decisionMatching.push({
        ...
        confidenceDifference:
          (selected1?.confidence || 0) - (selected2?.confidence || 0),
    ...
    return {
      ...
      timingComparison,
      decisionMatching,
      extraDecisionsInPath1:
        path1.decisionPoints.length > path2.decisionPoints.length
          ? path1.decisionPoints.slice(minLength).map((d) => d.id)
          : [],
      extraDecisionsInPath2:
        path2.decisionPoints.length > path1.decisionPoints.length
  ...
  identifyKeyDecisionPoints(pathId: string): string[] {
    ...
    if (path.decisionPoints.length > 0) {
      ...
      if (path.decisionPoints.length > 1) {
        const lastDecision =
          path.decisionPoints[path.decisionPoints.length - 1];
    ...
    if (path.decisionPoints.length > 2) {
      ...
      if (
        maxOptionsDecisionId &&
        !keyDecisions.includes(maxOptionsDecisionId)
      ) {
    ...
    if (path.confidenceOverTime.length > 2) {
      ...
      for (let i = 1; i < path.decisionPoints.length - 1; i++) {
        const prevConfidence = this.getDecisionConfidence(
          path.decisionPoints[i - 1],
        );
        const currConfidence = this.getDecisionConfidence(
          path.decisionPoints[i],
        );
        const nextConfidence = this.getDecisionConfidence(
          path.decisionPoints[i + 1],
        );
        ...
        if (
          ...
          Math.abs(prevDelta) + Math.abs(nextDelta) > maxDelta
        ) {
  ...
  mergeReasoningPaths(path1Id: string, path2Id: string): string {
    ...
    if (path1.agentId !== path2.agentId) {
      this.logger.warn(
        `Cannot merge paths from different agents: ${path1.agentId} and ${path2.agentId}`,
      );
    ...
    this.logger.info(
      `Merged reasoning paths ${path1Id} and ${path2Id} into ${mergedPathId}`,
    );
  ...
  tagReasoningPath(pathId: string, tags: Record<string, any>): boolean {
    ...
    this.logger.debug(`Tagged reasoning path ${pathId} with metadata`, {
      tags,
    });
  ...
  getReasoningPathStatistics(): Record<string, any> {
    const agentStats: Record<
      string,
      { pathCount: number; avgDecisions: number; avgConfidence: number }
    > = {};
    ...
    for (const path of this.reasoningPaths.values()) {
      ...
      agentStats[agentId].avgDecisions =
        (agentStats[agentId].avgDecisions *
          (agentStats[agentId].pathCount - 1) +
          path.decisionPoints.length) /
        agentStats[agentId].pathCount;

      // Calculate confidence
      const pathAvgConfidence =
        path.confidenceOverTime.length > 0
          ? path.confidenceOverTime.reduce(
