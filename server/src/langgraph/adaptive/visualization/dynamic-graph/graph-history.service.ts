...
export class GraphHistoryImpl implements GraphHistory {
  ...
  constructor(options: {
    ...
    if (!options.graphRenderer) {
      throw new Error(
        'Graph history service requires a real-time graph renderer',
      );
    }
  ...
  recordSnapshot(graphId: string, event?: string): string {
    try {
      ...
      if (previousSnapshot) {
        ...
        previousSnapshot.addedNodes.forEach((node) =>
          previousNodeMap.set(node.id, node),
        );
        previousSnapshot.updatedNodes.forEach((node) =>
          previousNodeMap.set(node.id, node),
        );
        ...
        previousSnapshot.addedEdges.forEach((edge) =>
          previousEdgeMap.set(edge.id, edge),
        );
        previousSnapshot.updatedEdges.forEach((edge) =>
          previousEdgeMap.set(edge.id, edge),
        );
      ...
      this.logger.debug(
        `Recorded graph snapshot ${snapshotId} for graph ${graphId}`,
        {
  ...
  getSnapshotsByGraph(
    ...
    if (startTime) {
      snapshots = snapshots.filter(
        (snapshot) => snapshot.timestamp.getTime() >= startTime.getTime(),
      );
    }

    if (endTime) {
      snapshots = snapshots.filter(
        (snapshot) => snapshot.timestamp.getTime() <= endTime.getTime(),
      );
    }
  ...
  getGraphStateAtTime(graphId: string, timestamp: Date): Graph {
    ...
    if (!closestSnapshot) {
      this.logger.warn(
        `No snapshots found before timestamp ${timestamp.toISOString()} for graph ${graphId}`,
      );
      throw new Error(
        `No snapshots found before the specified timestamp for graph ${graphId}`,
      );
    }
  ...
  revertToSnapshot(graphId: string, snapshotId: string): boolean {
    try {
      ...
      if (snapshot.graphId !== graphId) {
        this.logger.warn(
          `Snapshot ${snapshotId} belongs to graph ${snapshot.graphId}, not ${graphId}`,
        );
        return false;
      }
      ...
      this.logger.error(`Error reverting to snapshot:`, {
        error,
        graphId,
        snapshotId,
      });
  ...
  compareSnapshots(
    ...
    try {
      ...
      if (!snapshot1 || !snapshot2) {
        throw new Error(
          `Snapshot not found: ${!snapshot1 ? snapshot1Id : snapshot2Id}`,
        );
      }
      ...
      this.logger.error(`Error comparing snapshots:`, {
        error,
        snapshot1Id,
        snapshot2Id,
      });
      throw new Error(
        `Failed to compare snapshots ${snapshot1Id} and ${snapshot2Id}`,
      );
    }
  }
  ...
  private reconstructGraphFromSnapshot(snapshot: GraphHistorySnapshot): Graph {
    ...
    try {
      ...
      if (targetIndex === -1) {
        throw new Error(
          `Snapshot ${snapshot.id} not found in history for graph ${snapshot.graphId}`,
        );
      }
      ...
      this.logger.error(`Error reconstructing graph from snapshot:`, {
        error,
        snapshotId: snapshot.id,
      });
      throw new Error(
        `Failed to reconstruct graph from snapshot ${snapshot.id}`,
      );
    }
  }
  ...
  private cleanupOldSnapshots(graphId: string): void {
    ...
    this.logger.debug(
      `Cleaned up ${excessCount} old snapshots for graph ${graphId}`,
    );
  }
}
