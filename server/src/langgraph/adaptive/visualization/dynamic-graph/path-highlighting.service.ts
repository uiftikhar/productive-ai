...
export class PathHighlightingImpl implements PathHighlighting {
  ...
  private highlightedElements: Map<string, { nodeIds: string[]; edgeIds: string[] }> = new Map(); // graphId -> highlighted elements
  private highlightListeners: Map<string, ((highlights: { nodeIds: string[]; edgeIds: string[] }) => void)[]> = new Map();
   ...
   */
  highlightNode(graphId: string, nodeId: string, highlightType: string = 'default'): boolean {
    try {
      ...
      this.logger.debug(`Highlighted node ${nodeId} in graph ${graphId} with type ${highlightType}`);
      return true;
    } catch (error) {
      this.logger.error(`Error highlighting node ${nodeId} in graph ${graphId}:`, { error });
      return false;
    }
  }
   ...
   */
  highlightEdge(graphId: string, edgeId: string, highlightType: string = 'default'): boolean {
    try {
      ...
      this.logger.debug(`Highlighted edge ${edgeId} in graph ${graphId} with type ${highlightType}`);
      return true;
    } catch (error) {
      this.logger.error(`Error highlighting edge ${edgeId} in graph ${graphId}:`, { error });
      return false;
    }
  }
   ...
   */
  highlightPath(graphId: string, nodeIds: string[], edgeIds: string[], highlightType: string = 'default'): boolean {
    let success = true;
    ...
    if (success) {
      this.logger.info(`Highlighted path with ${nodeIds.length} nodes and ${edgeIds.length} edges in graph ${graphId}`);
    } else {
  ...
  clearHighlights(graphId: string): boolean {
    try {
      ...
      this.logger.error(`Error clearing highlights in graph ${graphId}:`, { error });
      return false;
    }
  }
    ...
    try {
      ...
      for (const edge of graph.edges) {
        if (edge.sourceId === taskId || edge.targetId === taskId) {
          ...
          const relatedNodeId = edge.sourceId === taskId ? edge.targetId : edge.sourceId;
      ...
      // Highlight the path
      const success = this.highlightPath(graphId, relatedNodes, relatedEdges, 'active-execution');
      
      if (success) {
        this.logger.info(`Highlighted active execution for task ${taskId} in graph ${graphId}`);
      }
      
      return success;
    } catch (error) {
      this.logger.error(`Error highlighting active execution for task ${taskId} in graph ${graphId}:`, { error });
      return false;
    }
  }
  ...
  getHighlightedElements(graphId: string): { nodeIds: string[]; edgeIds: string[] } {
    return this.highlightedElements.get(graphId) || { nodeIds: [], edgeIds: [] };
  }
  ...
  subscribeToHighlightUpdates(
    ...
    return () => {
      ...
      if (currentListeners) {
        ...
        this.logger.debug(`Removed highlight update listener for graph ${graphId}`);
      }
    };
  }
  ...
  private notifyHighlightListeners(graphId: string): void {
    ...
    for (const listener of listeners) {
      try {
        ...
        this.logger.error(`Error in highlight update listener for graph ${graphId}:`, { error });
      }
    }
  }
   ...
   */
  private addHighlightedElement(graphId: string, elementType: 'node' | 'edge', elementId: string, highlightType: string): void {
   ...
   */
  getElementsByHighlightType(graphId: string, highlightType: string): { nodeIds: string[]; edgeIds: string[] } {
