...
export class RealTimeGraphRendererImpl implements RealTimeGraphRenderer {
  ...
  private graphUpdateListeners: Map<string, ((graph: Graph) => void)[]> = new Map();
  private layoutEngines: Map<string, (nodes: GraphNode[], edges: GraphEdge[]) => void> = new Map();
  ...
  private registerDefaultLayoutEngines(): void {
    ...
    this.layoutEngines.set('force-directed', this.applyForceDirectedLayout.bind(this));
    
    // Hierarchical layout
    this.layoutEngines.set('hierarchical', this.applyHierarchicalLayout.bind(this));
   ...
   */
  initializeGraph(graphId: string, name: string, layout: string = 'force-directed'): string {
  ...
  addEdge(graphId: string, edge: Omit<GraphEdge, 'createdAt' | 'updatedAt'>): GraphEdge {
    ...
    const sourceNodeExists = graph.nodes.some(node => node.id === edge.sourceId);
    const targetNodeExists = graph.nodes.some(node => node.id === edge.targetId);
    
    if (!sourceNodeExists || !targetNodeExists) {
      this.logger.warn(`Cannot add edge with non-existent source or target node: ${edge.sourceId} -> ${edge.targetId}`);
      throw new Error(`Source or target node not found for edge: ${edge.sourceId} -> ${edge.targetId}`);
    }
    ...
    this.logger.debug(`Added edge ${newEdge.id} to graph ${graphId}: ${edge.sourceId} -> ${edge.targetId}`);
  ...
  updateNode(graphId: string, nodeId: string, updates: Partial<GraphNode>): GraphNode {
    ...
    if (nodeIndex === -1) {
      this.logger.warn(`Cannot update non-existent node: ${nodeId} in graph ${graphId}`);
  ...
  updateEdge(graphId: string, edgeId: string, updates: Partial<GraphEdge>): GraphEdge {
    ...
    if (edgeIndex === -1) {
      this.logger.warn(`Cannot update non-existent edge: ${edgeId} in graph ${graphId}`);
    ...
    if (updates.sourceId || updates.targetId) {
      const sourceNodeExists = graph.nodes.some(node => node.id === updatedEdge.sourceId);
      const targetNodeExists = graph.nodes.some(node => node.id === updatedEdge.targetId);
      
      if (!sourceNodeExists || !targetNodeExists) {
        this.logger.warn(`Cannot update edge with non-existent source or target node: ${updatedEdge.sourceId} -> ${updatedEdge.targetId}`);
        throw new Error(`Source or target node not found for updated edge: ${updatedEdge.sourceId} -> ${updatedEdge.targetId}`);
      }
    }
  ...
  removeNode(graphId: string, nodeId: string): boolean {
    ...
    if (!graph) {
      this.logger.warn(`Cannot remove node from non-existent graph: ${graphId}`);
      return false;
    }
    ...
    if (nodeIndex === -1) {
      this.logger.warn(`Cannot remove non-existent node: ${nodeId} from graph ${graphId}`);
      return false;
    }
    ...
    this.logger.debug(`Removed node ${nodeId} and its connected edges from graph ${graphId}`);
  ...
  removeEdge(graphId: string, edgeId: string): boolean {
    ...
    if (!graph) {
      this.logger.warn(`Cannot remove edge from non-existent graph: ${graphId}`);
      return false;
    }
    ...
    if (edgeIndex === -1) {
      this.logger.warn(`Cannot remove non-existent edge: ${edgeId} from graph ${graphId}`);
      return false;
    }
  ...
  private notifyGraphListeners(graphId: string): void {
    ...
    for (const listener of listeners) {
      try {
        ...
        this.logger.error(`Error in graph update listener for graph ${graphId}`, { error });
      }
    }
  }
