 ...
 */
export class CommunicationFlowVisualizationImpl implements CommunicationFlowVisualization {
  ...
  recordCommunication(communication: Omit<CommunicationEvent, 'id'>): string {
    ...
    this.logger.debug(`Recorded communication ${id} from ${communication.sourceAgentId} to ${communication.targetAgentId}`);
  ...
  getAgentCommunications(agentId: string, startTime?: Date, endTime?: Date): CommunicationEvent[] {
    const communicationIds = this.agentCommunicationIndex.get(agentId) || new Set<string>();
    ...
    // Sort by timestamp
    communications.sort((a, b) => a.timestamp.getTime() - b.timestamp.getTime());
  ...
  getTaskCommunications(taskId: string): CommunicationEvent[] {
    const communicationIds = this.taskCommunicationIndex.get(taskId) || new Set<string>();
   ...
   */
  visualizeCommunicationFlow(agentIds: string[], startTime?: Date, endTime?: Date): any {
    this.logger.debug(`Visualizing communication flow for ${agentIds.length} agents`);
    
    // Get all communications between the specified agents
    const agentCommunications: CommunicationEvent[] = [];
    
    for (const agentId of agentIds) {
      const communications = this.getAgentCommunications(agentId, startTime, endTime)
        .filter(comm => agentIds.includes(comm.sourceAgentId) && agentIds.includes(comm.targetAgentId));
      
      agentCommunications.push(...communications);
    }
    
    // Deduplicate communications
    const uniqueCommunications = this.deduplicateCommunications(agentCommunications);
    ...
    const nodes = agentIds.map(agentId => {
      const outgoingCount = Object.values(communicationFrequency[agentId] || {})
        .reduce((sum, count) => sum + count, 0);
        
      const incomingCount = agentIds
        .filter(id => id !== agentId)
        .reduce((sum, id) => sum + (communicationFrequency[id][agentId] || 0), 0);
    ...
    for (const sourceId of agentIds) {
      for (const targetId of agentIds) {
        if (sourceId !== targetId && communicationFrequency[sourceId][targetId] > 0) {
          edges.push({
            source: sourceId,
            target: targetId,
  ...
  analyzeConversationThreads(agentIds: string[]): any {
    this.logger.debug(`Analyzing conversation threads for ${agentIds.length} agents`);
    
    // Get all communications for the specified agents
    const allCommunications: CommunicationEvent[] = [];
    
    for (const agentId of agentIds) {
      const communications = this.getAgentCommunications(agentId)
        .filter(comm => agentIds.includes(comm.sourceAgentId) && agentIds.includes(comm.targetAgentId));
    ...
    // Deduplicate communications
    const uniqueCommunications = this.deduplicateCommunications(allCommunications);
    
    // Build conversation threads from root messages (those without responseToId)
    const rootMessages = uniqueCommunications.filter(comm => !comm.responseToId);
    
    // Recursively build thread structures
    const threads = rootMessages.map(rootMessage => this.buildThreadStructure(rootMessage));
    ...
    return {
      ...
      metrics: {
        ...
        averageThreadDepth: this.calculateAverage(threadAnalysis.map(t => t.depth)),
        averageParticipantCount: this.calculateAverage(threadAnalysis.map(t => t.participantCount)),
        averageThreadDuration: this.calculateAverage(threadAnalysis.map(t => t.duration))
      }
    };
  }
  ...
  identifyCommunicationBottlenecks(agentIds: string[]): any {
    this.logger.debug(`Identifying communication bottlenecks for ${agentIds.length} agents`);
    
    // Get agent communications
    const communicationsByAgent: Record<string, CommunicationEvent[]> = {};
    
    for (const agentId of agentIds) {
      communicationsByAgent[agentId] = this.getAgentCommunications(agentId)
        .filter(comm => agentIds.includes(comm.sourceAgentId) && agentIds.includes(comm.targetAgentId));
    }
    
    // Calculate incoming and outgoing message ratios
    const messageRatios: Record<string, { 
    ...
    for (const agentId of agentIds) {
      ...
      const incomingCount = allCommunications.filter(comm => comm.targetAgentId === agentId).length;
      const outgoingCount = allCommunications.filter(comm => comm.sourceAgentId === agentId).length;
    ...
    const bottlenecks = agentIds
      .filter(agentId => {
        ...
        return (
          ...
          (stats.incomingCount > 10 && stats.incomingCount > stats.outgoingCount * 1.5)
        );
      })
      .map(agentId => ({
        agentId,
   ...
   */
  private deduplicateCommunications(communications: CommunicationEvent[]): CommunicationEvent[] {
   ...
   */
  private getMessageTypeCounts(communications: CommunicationEvent[]): Record<string, number> {
  ...
  private buildThreadStructure(message: CommunicationEvent): any {
    const responses = Array.from(this.threadIndex.get(message.id) || new Set<string>())
      .map(id => this.communications.get(id))
      .filter(Boolean) as CommunicationEvent[];
      
    const responseStructures = responses.map(response => this.buildThreadStructure(response));
   ...
   */
  private collectThreadParticipants(thread: any, participantSet: Set<string>): void {
  ...
  private calculateThreadDepth(thread: any): number {
    ...
    const responseDepths = thread.responses.map((response: any) => this.calculateThreadDepth(response));
   ...
   */
  private findLatestTimestamp(thread: any, callback: (timestamp: Date) => void): void {
  ...
  private calculateAverageResponseDelay(agentId: string): number {
    const incomingMessages = Array.from(this.agentCommunicationIndex.get(agentId) || new Set<string>())
      .map(id => this.communications.get(id))
      .filter(comm => comm && comm.targetAgentId === agentId) as CommunicationEvent[];
      
    // Calculate response delays
    const responseDelays: number[] = [];
    
    for (const incomingMessage of incomingMessages) {
      const responses = Array.from(this.threadIndex.get(incomingMessage.id) || new Set<string>())
        .map(id => this.communications.get(id))
        .filter(comm => comm && comm.sourceAgentId === agentId) as CommunicationEvent[];
        
      for (const response of responses) {
        const delay = response.timestamp.getTime() - incomingMessage.timestamp.getTime();
