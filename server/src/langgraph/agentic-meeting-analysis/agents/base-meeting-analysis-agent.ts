 ...
 */
export class BaseMeetingAnalysisAgent extends EventEmitter implements IMeetingAnalysisAgent {
  ...
  protected memorySubscriptions: Map<string, ((value: any) => void)[]> = new Map();
  ...
  async initialize(config?: Record<string, any>): Promise<void> {
    try {
      ...
      this.logger.error(`Error initializing ${this.name} agent: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }
  ...
  async processTask(task: AnalysisTask): Promise<AgentOutput> {
    try {
      this.logger.info(`Agent ${this.name} processing task: ${task.id} (${task.type})`);
      
      // Validation
      if (!this.capabilities.has(task.type)) {
        throw new Error(`Agent ${this.name} does not have capability to handle task type: ${task.type}`);
      }
      ...
      this.logger.info(`Agent ${this.name} completed task ${task.id} with confidence: ${confidence}`);
      
      return output;
    } catch (error) {
      this.logger.error(`Error processing task ${task.id}: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }
  ...
  async receiveMessage(message: AgentMessage): Promise<void> {
    try {
      this.logger.debug(`Agent ${this.name} received message: ${message.id} from ${message.sender}`);
      ...
      this.logger.error(`Error receiving message: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }
  ...
  async readMemory(key: string, namespace: string = 'default'): Promise<any> {
    ...
    this.logger.warn('Memory service not implemented - using local memory cache only');
    return null;
  }
   ...
   */
  async writeMemory(key: string, value: any, namespace: string = 'default'): Promise<void> {
    const fullKey = `${namespace}:${key}`;
    ...
    if (this.memorySubscriptions.has(fullKey)) {
      ...
      for (const callback of callbacks) {
        try {
          ...
          this.logger.error(`Error in memory subscription callback: ${error instanceof Error ? error.message : String(error)}`);
        }
      }
    }
   ...
   */
  subscribeToMemory(key: string, callback: (value: any) => void, namespace: string = 'default'): void {
    const fullKey = `${namespace}:${key}`;
   ...
   */
  async requestAssistance(taskId: string, requestedExpertise: AgentExpertise): Promise<void> {
   ...
   */
  async provideAssistance(taskId: string, contribution: AgentOutput): Promise<void> {
    const message: AgentMessage = {
  ...
  async assessConfidence(output: any): Promise<ConfidenceLevel> {
    try {
      ...
      } else if (confidenceText.includes(ConfidenceLevel.MEDIUM.toUpperCase())) {
      ...
      this.logger.error(`Error assessing confidence: ${error instanceof Error ? error.message : String(error)}`);
      return ConfidenceLevel.UNCERTAIN;
    }
  }
  ...
  async explainReasoning(output: any): Promise<string> {
    try {
      ...
      this.logger.error(`Error explaining reasoning: ${error instanceof Error ? error.message : String(error)}`);
   ...
   */
  protected async callLLM(instruction: string, content: string): Promise<string> {
    try {
      ...
      this.logger.error(`Error calling LLM: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }
...
${Array.from(this.capabilities).map(capability => `- ${capability.replace('_', ' ')}`).join('\n')}
