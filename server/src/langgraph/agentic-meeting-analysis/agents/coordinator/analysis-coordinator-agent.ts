 ...
 */
export class AnalysisCoordinatorAgent extends BaseMeetingAnalysisAgent implements IAnalysisCoordinatorAgent {
  private maxTeamSize: number;
  private qualityThreshold: number;
  private specialistRegistry: Map<string, {
    agentId: string;
  ...
  constructor(config: AnalysisCoordinatorAgentConfig) {
    ...
    this.logger.info(`Initialized ${this.name} with max team size: ${this.maxTeamSize}`);
  }
  ...
  private async analyzeMeetingCharacteristics(transcript: string): Promise<{
    ...
    const response = await this.callLLM('Analyze meeting characteristics', prompt);
    ...
    try {
      ...
      for (const expertise of characteristics.requiredExpertise) {
        if (Object.values(AgentExpertise).includes(expertise as AgentExpertise)) {
      ...
      return {
        ...
        technicalLevel: Math.min(Math.max(characteristics.technicalLevel, 0), 1),
        controversyLevel: Math.min(Math.max(characteristics.controversyLevel, 0), 1),
        actionOriented: Boolean(characteristics.actionOriented),
        requiredExpertise: validExpertise
      };
      
    } catch (error) {
      this.logger.error(`Error parsing meeting characteristics: ${error instanceof Error ? error.message : String(error)}`);
   ...
   */
  async formTeam(analysisGoal: AnalysisGoalType, transcript: string): Promise<string[]> {
    this.logger.info(`Forming team for analysis goal: ${analysisGoal}`);
    
    // Analyze meeting characteristics
    const characteristics = await this.analyzeMeetingCharacteristics(transcript);
    ...
    for (const expertise of requiredExpertise) {
      ...
      if (expertAgents.length > 0) {
        ...
        this.specialistRegistry.get(expertAgents[0].agentId)!.availability = false;
      }
    }
    ...
    this.logger.info(`Formed team with ${selectedAgents.length} agents for goal: ${analysisGoal}`);
   ...
   */
  async createWorkflow(analysisGoal: AnalysisGoalType): Promise<AnalysisTask[]> {
    ...
    this.logger.info(`Created workflow with ${tasks.length} tasks for goal: ${analysisGoal}`);
  ...
  async synthesizeResults(taskIds: string[]): Promise<AgentOutput> {
    ...
    for (const taskId of taskIds) {
      ...
      if (taskData && taskData.output && taskData.status === AnalysisTaskStatus.COMPLETED) {
    ...
    for (const [taskId, output] of Object.entries(taskOutputs)) {
      ...
      synthesisPrompt += typeof output === 'string' 
        ? output 
...
Format your response as a structured JSON object with these sections.`;
  ...
  async processTask(task: AnalysisTask): Promise<AgentOutput> {
    ...
    if (task.type === AnalysisGoalType.FULL_ANALYSIS) {
      ...
      // Form team
      const teamMembers = await this.formTeam(AnalysisGoalType.FULL_ANALYSIS, transcript);
      
      // Create workflow
      const workflowTasks = await this.createWorkflow(AnalysisGoalType.FULL_ANALYSIS);
      
      // Assign initial tasks (those without dependencies)
      for (const workflowTask of workflowTasks) {
        if (!workflowTask.dependencies || workflowTask.dependencies.length === 0) {
          // Find an appropriate agent for this task
          const suitableAgents = Array.from(this.specialistRegistry.values())
            .filter(agent => 
      ...
      return {
        ...
        reasoning: 'Successfully created and initiated the full analysis workflow',
        metadata: {
      ...
      const tasksData = await this.readMemory('tasks', 'analysis') || {};
      const completedTaskIds = Object.keys(tasksData).filter(taskId => 
        tasksData[taskId].status === AnalysisTaskStatus.COMPLETED
      );
    ...
    throw new Error(`Coordinator agent cannot directly handle task type: ${task.type}`);
  }
  ...
  private async handleAgentRegistration(message: AgentMessage): Promise<void> {
    if (message.content?.messageType === 'AGENT_REGISTRATION') {
      ...
      this.logger.info(`Registering agent ${name} (${agentId}) with expertise: ${expertise.join(', ')}`);
  ...
  private async handleTaskCompletion(message: AgentMessage): Promise<void> {
    if (message.content?.messageType === 'TASK_COMPLETED') {
      ...
      this.logger.info(`Received task completion for task ${taskId} from agent ${message.sender}`);
