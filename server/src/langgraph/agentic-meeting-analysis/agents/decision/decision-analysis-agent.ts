 ...
 */
export class DecisionAnalysisAgent extends BaseMeetingAnalysisAgent implements ISpecialistAnalysisAgent {
  ...
  constructor(config: DecisionAnalysisAgentConfig) {
    ...
    this.enableStakeholderIdentification = config.enableStakeholderIdentification !== false;
    ...
    this.logger.info(`Initialized ${this.name} with features: rationales=${this.enableRationaleExtraction}, stakeholders=${this.enableStakeholderIdentification}`);
  }
  ...
  async processTask(task: AnalysisTask): Promise<AgentOutput> {
    ...
    if (task.type !== AnalysisGoalType.EXTRACT_DECISIONS) {
      throw new Error(`Decision Analysis Agent cannot process task type: ${task.type}`);
    }
    ...
    try {
      ...
      const decisionAnalysis = {
        ...
        metadata: {
          ...
          withRationales: decisions.filter(decision => decision.rationale).length,
          withStakeholders: decisions.filter(decision => decision.stakeholders && decision.stakeholders.length > 0).length
        }
      };
      ...
      this.logger.error(`Error processing decision analysis: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }
   ...
   */
  private async extractDecisions(transcript: MeetingTranscript, metadata: any): Promise<Array<{
    id: string;
    description: string;
    ...
    const response = await this.callLLM('Extract decisions from transcript', prompt);
    ...
    try {
      ...
      this.logger.error(`Error parsing decision extraction response: ${error instanceof Error ? error.message : String(error)}`);
   ...
   */
  private async extractRationales(decisions: Array<any>, transcript: MeetingTranscript): Promise<void> {
   ...
   */
  private async evaluateDecisionQuality(decisions: Array<any>, transcript: MeetingTranscript): Promise<void> {
    // Implementation left as a stub
  }
   ...
   */
  private async analyzeDecisionPatterns(decisions: Array<any>, transcript: MeetingTranscript): Promise<void> {
    // Implementation left as a stub
  }
  ...
  async analyzeTranscriptSegment(segment: string, context?: any): Promise<AgentOutput> {
    ...
    const response = await this.callLLM('Analyze segment for decisions', prompt);
    ...
    try {
      ...
      if (segmentAnalysis.decisions) {
        segmentAnalysis.decisions = segmentAnalysis.decisions.map((decision: any) => ({
          ...decision,
          id: decision.id || `decision-${uuidv4().substring(0, 8)}`
        }));
      }
  ...
  async mergeAnalyses(analyses: AgentOutput[]): Promise<AgentOutput> {
    ...
    try {
      ...
      this.logger.error(`Error merging decision analyses: ${error instanceof Error ? error.message : String(error)}`);
