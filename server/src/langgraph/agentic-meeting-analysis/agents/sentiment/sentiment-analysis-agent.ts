 ...
 */
export class SentimentAnalysisAgent extends BaseMeetingAnalysisAgent implements ISpecialistAnalysisAgent {
  ...
  constructor(config: SentimentAnalysisAgentConfig) {
    ...
    this.enableParticipantSentiment = config.enableParticipantSentiment !== false;
    this.enableSentimentShiftTracking = config.enableSentimentShiftTracking !== false;
    ...
    this.logger.info(`Initialized ${this.name} with features: topicSentiment=${this.enableTopicSentiment}, participantSentiment=${this.enableParticipantSentiment}`);
  }
  ...
  async processTask(task: AnalysisTask): Promise<AgentOutput> {
    ...
    if (task.type !== AnalysisGoalType.ANALYZE_SENTIMENT) {
      throw new Error(`Sentiment Analysis Agent cannot process task type: ${task.type}`);
    }
    ...
    try {
      ...
      if (this.enableParticipantSentiment) {
        participantSentiment = await this.analyzeParticipantSentiment(transcript, metadata);
      }
      ...
      if (this.enableAlignmentAnalysis) {
        participantAlignment = await this.analyzeParticipantAlignment(transcript, metadata, topics);
      }
      ...
      this.logger.error(`Error processing sentiment analysis: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }
   ...
   */
  private async analyzeOverallSentiment(transcript: MeetingTranscript): Promise<{
    ...
    try {
      ...
      this.logger.error(`Error parsing overall sentiment: ${error instanceof Error ? error.message : String(error)}`);
  ...
  private async analyzeParticipantSentiment(
    ...
    for (const participant of participants) {
      ...
      try {
        const response = await this.callLLM(`Analyze sentiment for ${participant.name || speakerId}`, prompt);
        ...
        this.logger.error(`Error analyzing sentiment for participant ${speakerId}: ${error instanceof Error ? error.message : String(error)}`);
  ...
  private async analyzeTopicSentiment(
    ...
    for (let i = 0; i < topics.length; i += batchSize) {
      ...
      const topicsWithSegments = await Promise.all(topicBatch.map(async (topic) => {
        // Get segments related to this topic
        const topicSegments: string[] = topic.segments || [];
        
        // Get up to 5 segments for context
        const sampleSegments = topicSegments.length > 5 
          ? topicSegments.slice(0, 5) 
          : topicSegments;
        
        const segmentTexts = sampleSegments.map(segId => {
      ...
      const prompt = `
        ...
        ${topicsWithSegments.map((topic, index) => `
      ...
      try {
        ...
        this.logger.error(`Error analyzing topic sentiment: ${error instanceof Error ? error.message : String(error)}`);
   ...
   */
  private async trackSentimentShifts(transcript: MeetingTranscript): Promise<Array<{
    startTime: number;
    endTime: number;
    ...
    const chunkSentiments = await Promise.all(chunks.map(async (chunk, index) => {
      ...
      try {
        const response = await this.callLLM(`Analyze sentiment for chunk ${index + 1}`, prompt);
        const analysis = JSON.parse(response);
        ...
        this.logger.error(`Error analyzing chunk sentiment: ${error instanceof Error ? error.message : String(error)}`);
  ...
  async analyzeTranscriptSegment(segment: string, context?: any): Promise<AgentOutput> {
    ...
    try {
      ...
      this.logger.error(`Error analyzing segment sentiment: ${error instanceof Error ? error.message : String(error)}`);
