...
export class ApiCompatibilityService implements IApiCompatibilityLayer {
  ...
  convertLegacyToAgenticRequest(legacyRequest: LegacyMeetingAnalysisRequest): AgenticMeetingAnalysisRequest {
    this.logger.debug(`Converting legacy request for meeting ${legacyRequest.meetingId} to agentic format`);
    ...
    // Map participants
    const participants = legacyRequest.participantIds?.map(id => ({
      id,
  ...
  convertAgenticToLegacyResponse(agenticResponse: AgenticMeetingAnalysisResponse): LegacyMeetingAnalysisResponse {
    this.logger.debug(`Converting agentic response for meeting ${agenticResponse.meetingId} to legacy format`);
   ...
   */
  async processLegacyRequest(request: LegacyMeetingAnalysisRequest): Promise<LegacyMeetingAnalysisResponse> {
    this.logger.info(`Processing legacy request for meeting ${request.meetingId}`);
    ...
    if (agenticModeEnabled) {
      ...
      this.logger.info('Agentic mode is disabled, delegating to legacy implementation');
      
      throw new Error('Legacy implementation is not available - please enable agentic mode');
    }
  }
   ...
   */
  async processAgenticRequest(request: AgenticMeetingAnalysisRequest): Promise<AgenticMeetingAnalysisResponse> {
    this.logger.info(`Processing agentic request for meeting ${request.meetingId}`);
    ...
    throw new Error('Agentic implementation not yet connected - this is a placeholder');
  }
   ...
   */
  async getLegacyAnalysisResults(meetingId: string): Promise<LegacyMeetingAnalysisResponse> {
  ...
  async isAgenticMode(): Promise<boolean> {
    return this.featureFlags.get(ApiCompatibilityService.FEATURE_FLAG_KEY) ?? false;
  }
