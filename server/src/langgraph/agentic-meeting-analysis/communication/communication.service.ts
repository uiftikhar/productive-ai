 ...
 */
export class CommunicationService extends EventEmitter implements ICommunicationService {
  ...
  private agentCallbacks: Map<string, (message: AgentMessage) => Promise<void>> = new Map();
  private topicSubscriptions: Map<string, TopicSubscriptionOptions[]> = new Map();
   ...
   */
  async sendMessage(message: AgentMessage, options?: MessagePublishOptions): Promise<string> {
    this.logger.debug(`Sending message ${message.id} from ${message.sender} to ${Array.isArray(message.recipients) ? message.recipients.join(', ') : message.recipients}`);
    ...
    if (message.recipients === 'broadcast') {
      ...
      for (const agentId of agents) {
        if (agentId !== message.sender) { // Don't deliver to self
   ...
   */
  async broadcastMessage(message: Omit<AgentMessage, 'recipients'>, options?: MessagePublishOptions): Promise<string> {
   ...
   */
  async createChannel(channelDefinition: Omit<CommunicationChannel, 'id' | 'created'>): Promise<string> {
   ...
   */
  async addParticipantToChannel(channelId: string, agentId: string): Promise<void> {
   ...
   */
  async removeParticipantFromChannel(channelId: string, agentId: string): Promise<void> {
    const channel = this.channels.get(channelId);
   ...
   */
  async publishToTopic(topic: string, message: Omit<AgentMessage, 'recipients'>, options?: MessagePublishOptions): Promise<string> {
  ...
  async subscribeToTopic(options: TopicSubscriptionOptions): Promise<void> {
    this.logger.debug(`Agent ${options.agentId} subscribing to topic: ${options.topic}`);
    ...
    const filteredSubscriptions = subscriptions.filter(sub => sub.agentId !== options.agentId);
  ...
  async unsubscribeFromTopic(agentId: string, topic: string): Promise<void> {
    ...
    const filteredSubscriptions = subscriptions.filter(sub => sub.agentId !== agentId);
   ...
   */
  async getMessageDeliveryStatus(messageId: string): Promise<Record<string, MessageDeliveryStatus>> {
   ...
   */
  async confirmMessageReceipt(messageId: string, agentId: string, status: MessageDeliveryStatus): Promise<void> {
    this.logger.debug(`Confirming message ${messageId} receipt by agent ${agentId} with status: ${status}`);
    
    const pendingMessage = this.pendingMessages.get(messageId);
    
    if (!pendingMessage) {
      this.logger.warn(`Cannot confirm receipt: message ${messageId} not found in pending messages`);
      return;
    }
    ...
    if (pendingMessage.deliveryStatus[agentId]) {
      ...
      const allDelivered = Object.values(pendingMessage.deliveryStatus).every(
        s => s === MessageDeliveryStatus.DELIVERED || s === MessageDeliveryStatus.READ
      );
   ...
   */
  async registerAgent(agentId: string, callback: (message: AgentMessage) => Promise<void>): Promise<void> {
    ...
    for (const [messageId, pendingMessage] of this.pendingMessages.entries()) {
      ...
      if (deliveryStatus[agentId] === MessageDeliveryStatus.PENDING) {
        try {
          ...
          if (Array.isArray(message.recipients) && message.recipients.includes(agentId)) {
            await callback(message);
            deliveryStatus[agentId] = MessageDeliveryStatus.DELIVERED;
          } 
          // Message is a broadcast
          else if (message.recipients === 'broadcast' && message.sender !== agentId) {
            await callback(message);
            deliveryStatus[agentId] = MessageDeliveryStatus.DELIVERED;
          }
        } catch (error) {
          this.logger.error(`Error delivering pending message ${messageId} to agent ${agentId}: ${error instanceof Error ? error.message : String(error)}`);
  ...
  async getMessageHistory(options: {
    ...
    if (options.agentId) {
      messages = messages.filter(msg => 
        ...
        (Array.isArray(msg.recipients) && msg.recipients.includes(options.agentId!)) ||
      ...
      if (channel) {
        messages = messages.filter(msg => {
          if (Array.isArray(msg.recipients)) {
            ...
            return msg.recipients.every(r => channel.participants.includes(r)) && 
    ...
    if (options.topic) {
      messages = messages.filter(msg => 
        msg.metadata?.topic === options.topic
      );
    }
    ...
    for (const pendingMessage of this.pendingMessages.values()) {
      ...
      if (
        (deliveryStatus[agentId] === MessageDeliveryStatus.PENDING || 
         deliveryStatus[agentId] === MessageDeliveryStatus.FAILED) && 
        (
          (Array.isArray(message.recipients) && message.recipients.includes(agentId)) || 
  ...
  async getMetrics(): Promise<{
    ...
    for (const channel of this.channels.values()) {
      const channelMessages = await this.getMessageHistory({ channelId: channel.id });
  ...
  private async deliverMessage(message: AgentMessage): Promise<void> {
    ...
    if (Array.isArray(message.recipients)) {
      ...
      for (const recipientId of message.recipients) {
        if (pendingMessage.deliveryStatus[recipientId] === MessageDeliveryStatus.PENDING) {
          const callback = this.agentCallbacks.get(recipientId);
          
          if (callback) {
            try {
              await callback(message);
              pendingMessage.deliveryStatus[recipientId] = MessageDeliveryStatus.DELIVERED;
            } catch (error) {
              this.logger.error(`Error delivering message ${message.id} to agent ${recipientId}: ${error instanceof Error ? error.message : String(error)}`);
              pendingMessage.deliveryStatus[recipientId] = MessageDeliveryStatus.FAILED;
            }
          }
        }
      }
      ...
      for (const [agentId, callback] of this.agentCallbacks.entries()) {
        if (agentId !== message.sender && pendingMessage.deliveryStatus[agentId] === MessageDeliveryStatus.PENDING) {
          try {
            await callback(message);
            pendingMessage.deliveryStatus[agentId] = MessageDeliveryStatus.DELIVERED;
          } catch (error) {
            this.logger.error(`Error broadcasting message ${message.id} to agent ${agentId}: ${error instanceof Error ? error.message : String(error)}`);
            pendingMessage.deliveryStatus[agentId] = MessageDeliveryStatus.FAILED;
          }
        }
      }
    ...
    const allDelivered = Object.values(pendingMessage.deliveryStatus).every(
      status => status === MessageDeliveryStatus.DELIVERED || status === MessageDeliveryStatus.READ
    );
  ...
  private processExpiredMessages(): void {
    ...
    for (const [messageId, pendingMessage] of this.pendingMessages.entries()) {
      if (pendingMessage.expiresAt && pendingMessage.expiresAt < now) {
        ...
        for (const [recipientId, status] of Object.entries(pendingMessage.deliveryStatus)) {
          if (status === MessageDeliveryStatus.PENDING) {
            pendingMessage.deliveryStatus[recipientId] = MessageDeliveryStatus.FAILED;
          }
        }
        
        this.logger.warn(`Message ${messageId} expired before delivery to all recipients`);
