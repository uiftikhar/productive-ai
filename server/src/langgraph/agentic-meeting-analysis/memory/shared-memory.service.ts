...
export class SharedMemoryService extends EventEmitter implements ISharedMemory {
  ...
  private subscriptionCallbacks: Map<string, ((notification: MemoryUpdateNotification) => void)[]> = new Map();
  ...
  async initialize(): Promise<void> {
    ...
    if (this.persistenceEnabled) {
      try {
        ...
        this.logger.warn(`Failed to load persisted memory: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
   ...
   */
  async read(key: string, namespace: string = this.defaultNamespace, agentId: string = 'system'): Promise<any> {
   ...
   */
  async delete(key: string, namespace: string = this.defaultNamespace, agentId: string = 'system'): Promise<void> {
    ...
    if (!entry) {
      this.logger.debug(`Key ${key} not found in namespace ${namespace} for deletion`);
      return;
    }
  ...
  async query(options: MemoryQueryOptions): Promise<Record<string, any>> {
    this.logger.debug(`Querying memory with options: ${JSON.stringify(options)}`);
    
    const results: Record<string, any> = {};
    const namespaces = options.namespaces || [this.defaultNamespace];
    const keyPattern = options.keyPattern ? (typeof options.keyPattern === 'string' ? new RegExp(options.keyPattern) : options.keyPattern) : null;
   ...
   */
  async listKeys(namespace: string = this.defaultNamespace, pattern?: string | RegExp): Promise<string[]> {
    const keys: string[] = [];
  ...
  subscribe(
    ...
    this.logger.debug(`Agent ${agentId} subscribing to key ${key} in namespace ${namespace}`);
   ...
   */
  unsubscribe(key: string, namespace: string = this.defaultNamespace, agentId: string): void {
    this.logger.debug(`Agent ${agentId} unsubscribing from key ${key} in namespace ${namespace}`);
   ...
   */
  async getHistory(key: string, namespace: string = this.defaultNamespace, limit?: number): Promise<MemoryEntry['versions']> {
    this.logger.debug(`Getting history for key ${key} in namespace ${namespace}`);
   ...
   */
  async revertTo(key: string, namespace: string = this.defaultNamespace, timestamp: number): Promise<void> {
    this.logger.debug(`Reverting key ${key} in namespace ${namespace} to timestamp ${timestamp}`);
    
    const fullKey = this.getFullKey(namespace, key);
    const entry = this.entries.get(fullKey);
    
    if (!entry) {
      throw new Error(`Cannot revert: key ${key} does not exist in namespace ${namespace}`);
    }
    ...
    if (!targetVersion) {
      throw new Error(`Cannot revert: no version exists at or before timestamp ${timestamp}`);
    }
    ...
    await this.write(
      ...
      namespace, 
      'system', 
      { revertedFrom: timestamp, originalAgentId: targetVersion.agentId }
    );
    
    this.logger.info(`Successfully reverted key ${key} to timestamp ${timestamp}`);
  }
  
  /**
   * Detect conflicts between operations
   */
  async detectConflicts(operations: MemoryOperation[]): Promise<MemoryConflict[]> {
    this.logger.debug(`Detecting conflicts among ${operations.length} operations`);
    ...
    for (const [fullKey, writes] of Object.entries(writesByKey)) {
      if (writes.length > 1) {
        ...
        for (let i = 0; i < writes.length - 1; i++) {
          if (writes[i+1].timestamp - writes[i].timestamp < 1000 && writes[i].agentId !== writes[i+1].agentId) {
    ...
    for (const op of operations) {
      if (op.type === MemoryOperationType.READ) {
        ...
        if (priorWrites.length > 0) {
          ...
          const latestWrite = priorWrites.reduce((latest, current) => 
   ...
   * Resolve a conflict
   */
  async resolveConflict(conflict: MemoryConflict, resolution: any): Promise<void> {
    this.logger.info(`Resolving conflict for key ${conflict.key} in namespace ${conflict.namespace}`);
  ...
  async getStats(): Promise<{
    ...
    const averageVersionsPerKey = totalEntries > 0 ? totalVersions / totalEntries : 0;
    ...
    for (const agentId of entry.subscribers) {
      ...
      for (const callback of callbacks) {
        try {
          ...
          this.logger.error(`Error in subscription callback for agent ${agentId}: ${error instanceof Error ? error.message : String(error)}`);
        }
      }
    }
  }
