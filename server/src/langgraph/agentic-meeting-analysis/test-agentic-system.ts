...
async function testSharedMemory(logger: ConsoleLogger) {
  ...
  await memory.write(testKey, { ...testValue, priority: 10 }, 'test-namespace', 'test-agent');
...
async function testStateRepository(logger: ConsoleLogger) {
  ...
  console.log('Meeting state created:', meetingState.meetingId, meetingState.status);
  ...
  await state.updateResults(meetingId, {
    ...
    summary: {
      ...
      detailed: 'This is a more detailed summary with all the important points.'
    },
    topics: [{ 
      id: 'topic1', 
      name: 'Test Topic', 
...
async function testCommunication(logger: ConsoleLogger) {
  ...
  await communication.registerAgent('agent1', async (message) => {
    ...
    console.log(`Agent1 received message: ${message.id} from ${message.sender}`);
  });
  
  await communication.registerAgent('agent2', async (message) => {
    agent2Messages.push(message);
    console.log(`Agent2 received message: ${message.id} from ${message.sender}`);
  });
...
async function testApiCompatibility(logger: ConsoleLogger) {
  ...
  const legacyRequest = {
    ...
    transcript: 'This is a test transcript for the meeting. It includes some discussion and action items.',
    title: 'Test Meeting',
  ...
  const agenticRequest = compatibility.convertLegacyToAgenticRequest(legacyRequest);
  ...
  const hasTopicGoal = agenticRequest.goals?.includes(AnalysisGoalType.EXTRACT_TOPICS);
  const hasActionItemGoal = agenticRequest.goals?.includes(AnalysisGoalType.EXTRACT_ACTION_ITEMS);
  const hasSentimentGoal = agenticRequest.goals?.includes(AnalysisGoalType.ANALYZE_SENTIMENT);
  ...
  // Test error mapping
  const mappedError = compatibility.mapAgenticErrorToLegacyFormat(new Error('Test error'));
