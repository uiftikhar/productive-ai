...
async function testSharedResponsibilityService() {
  ...
  const mockCollaborativeTaskService = new MockCollaborativeTaskService(
    mockHierarchicalTaskService,
  );
  ...
  try {
    ...
    const task = await mockHierarchicalTaskService.createTask({
      ...
      description:
        'Create a service for managing shared responsibilities in peer tasks',
    ...
    const secondaryResp =
      await sharedResponsibilityService.assignResponsibility(
        task.id,
        'agent-2',
    ...
    const reviewedContrib =
      await sharedResponsibilityService.reviewContribution(
        contrib1.id,
        'agent-3',
        'approved',
      );
    logger.info(
      `Reviewed contribution: ${reviewedContrib.id}, status: ${reviewedContrib.reviewStatus}`,
    );
    ...
    const agent1Summary =
      await sharedResponsibilityService.getAgentContributionSummary(
        task.id,
        'agent-1',
      );
    ...
    const progress =
      await sharedResponsibilityService.getTaskResponsibilityProgress(task.id);
    ...
    const autoAssignments =
      await sharedResponsibilityService.autoDistributeResponsibilities(
        newTask.id,
        agents,
        'balanced',
      );
    ...
    for (const assignment of autoAssignments) {
      logger.info(
        `- ${assignment.agentId}: ${assignment.type} (${assignment.percentage}%)`,
      );
    }
