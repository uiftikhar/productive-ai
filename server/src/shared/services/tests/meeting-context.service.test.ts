...
import { MeetingContextService } from '../user-context/meeting-context.service';
...
describe('MeetingContextService', () => {
  ...
  beforeEach(() => {
    ...
    (BaseContextService.prototype as any).storeUserContext = jest
      .fn()
      .mockImplementation(
        (userId, content, embeddings, metadata) =>
          `meeting-context-${Math.floor(Math.random() * 1000)}`,
      );
  ...
  describe('storeMeetingContent', () => {
    test('should store meeting content with required parameters', async () => {
      ...
      expect(
        BaseContextService.prototype.storeUserContext,
      ...
      expect((service as any).meetingContents[userId][meetingId].content).toBe(
        content,
      );
    });
  ...
  describe('storeDecision', () => {
    test('should store decision with required parameters', async () => {
      ...
      expect((service as any).decisions[userId][meetingId][0].description).toBe(
        decision,
      );
    });
  ...
  describe('storeActionItem', () => {
    test('should store action item with required parameters', async () => {
      ...
      expect(
        (service as any).actionItems[userId][meetingId][0].description,
      ).toBe(actionItem);
      expect((service as any).actionItems[userId][meetingId][0].assignee).toBe(
        assigneeId,
      );
    });
  ...
  describe('storeQuestion', () => {
    test('should store question with required parameters', async () => {
      ...
      expect((service as any).questions[userId][meetingId][0].question).toBe(
        question,
      );
      expect((service as any).questions[userId][meetingId][0].isAnswered).toBe(
        false,
      );
    });
    ...
    test('should store an answered question with answer context ID', async () => {
      ...
      expect(
        (service as any).questions[userId][meetingId][0].answerContextId,
      ).toBe(answerContextId);
  ...
  describe('markQuestionAsAnswered', () => {
    ...
    test('should throw error if question not found', async () => {
      ...
      // Act & Assert
      await expect(
        service.markQuestionAsAnswered(userId, questionId, 'context-456'),
      ).rejects.toThrow();
    });
  });
  ...
  describe('updateActionItemStatus', () => {
    test('should update action item status to completed', async () => {
      ...
      // Assert
      expect(result).toBe(true);
      expect((service as any).actionItems[userId][meetingId][0].status).toBe(
        'completed',
      );
    });
