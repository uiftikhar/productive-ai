    ...
    .mockImplementation(
      (indexName: string, vectors: PineconeVector[], namespace?: string) => {
        ...
        vectors.forEach((vector) => {
          const existingIndex = this.vectors[indexName].findIndex(
            (v) => v.id === vector.id,
          );
          if (existingIndex >= 0) {
    ...
    .mockImplementation(
      (indexName: string, ids: string[], namespace?: string) => {
        if (this.vectors[indexName]) {
          ...
          this.vectors[indexName] = this.vectors[indexName].filter(
            (vector) => !ids.includes(vector.id),
          );
          return Promise.resolve({
            deletedCount: initialCount - this.vectors[indexName].length,
          });
        }
