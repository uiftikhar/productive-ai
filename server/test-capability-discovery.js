...
} = require('./dist/agents/services/capability-negotiation.service');
const {
  CapabilityTaxonomy,
  CapabilityLevel,
} = require('./dist/agents/interfaces/capability-discovery.interface');

async function runTest() {
  console.log(
    '=== Testing Enhanced Capability Discovery and Negotiation ===\n',
  );
      ...
      compatibilities: [
        {
          ...
          strength: 0.9,
          description:
            'Web research provides raw data for information synthesis',
        },
        {
          type: 'prerequisite',
  ...
  registry.registerCapability(
    {
      ...
      description:
        'Ability to combine and analyze information from multiple sources',
      level: CapabilityLevel.STANDARD,
      taxonomy: [
        CapabilityTaxonomy.REASONING,
        CapabilityTaxonomy.SUMMARIZATION,
      ],
      compatibilities: [
        {
          type: 'complementary',
          ...
          strength: 0.8,
          description:
            'Information synthesis provides refined inputs for content creation',
        },
      ],
    },
    'analysis-agent-1',
  ...
  registry.registerCapability(
    {
      ...
      taxonomy: [
        CapabilityTaxonomy.CONTENT_CREATION,
        CapabilityTaxonomy.GENERATION,
      ],
      compatibilities: [
        {
          type: 'complementary',
          ...
          strength: 0.7,
          description:
            'Creative ideation enhances content creation with novel ideas',
        },
      ],
    },
    'writer-agent-1',
  ...
  registry.registerCapability(
    {
      ...
      taxonomy: [
        CapabilityTaxonomy.GENERATION,
        CapabilityTaxonomy.META_COGNITION,
      ],
    },
    'creative-agent-1',
  );
      ...
      compatibilities: [
        {
          ...
          strength: 0.9,
          description:
            'Code generation is complemented by code review for quality assurance',
        },
      ],
    },
    'coder-agent-1',
  ...
  console.log(
    'Registered capabilities for 7 agents with various complementary relationships\n',
  );
  ...
  const similarToContentCreation =
    registry.getSimilarCapabilities('content_creation');
  ...
  const complementaryToResearch = registry.getComplementaryCapabilities([
    'web_research',
  ]);
  console.log(
    'Capabilities complementary to web_research:',
    complementaryToResearch,
  );

  const complementaryToCoding = registry.getComplementaryCapabilities([
    'code_generation',
  ]);
  console.log(
    'Capabilities complementary to code_generation:',
    complementaryToCoding,
  );
  ...
  console.log(
    `  - Composition score: ${researchWritingCombo.compositionScore.toFixed(2)}`,
  );
  console.log(
    `  - Complementarity score: ${researchWritingCombo.complementarityScore.toFixed(2)}`,
  );
  console.log(
    `  - Taxonomic coverage: ${researchWritingCombo.taxonomicCoverageScore.toFixed(2)}`,
  );
  console.log(
    `  - Missing critical capabilities:`,
    researchWritingCombo.missingCriticalCapabilities,
  );
  console.log(
    `  - Suggested additions:`,
    researchWritingCombo.suggestedAdditions,
  );
  ...
  console.log(
    `  - Composition score: ${codingCombo.compositionScore.toFixed(2)}`,
  );
  console.log(
    `  - Complementarity score: ${codingCombo.complementarityScore.toFixed(2)}`,
  );
  console.log(
    `  - Taxonomic coverage: ${codingCombo.taxonomicCoverageScore.toFixed(2)}`,
  );
  console.log(
    `  - Missing critical capabilities:`,
    codingCombo.missingCriticalCapabilities,
  );
  ...
  researchTeam.providers.forEach(({ providerId, capabilities, score }) => {
    console.log(
      `    - ${providerId} (score: ${score.toFixed(2)}, capabilities: ${capabilities.join(', ')})`,
    );
  });
  ...
  console.log(
    `Created capability inquiry ${inquiry.inquiryId} for web_research`,
  );
  ...
  console.log(
    `  - Available providers: ${result.availableProviders.join(', ')}`,
  );
