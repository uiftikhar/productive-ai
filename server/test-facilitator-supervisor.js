...
} = require('./dist/agents/specialized/facilitator-supervisor-agent');
...
async function createFacilitatorSupervisor(agentRegistry) {
  ...
  const supervisor = new FacilitatorSupervisorAgent({
    ...
    description:
      'Facilitates collaborative team decision-making and task execution',
    logger,
    agentRegistry,
  });
 ...
 */
async function demonstrateCollaborativeTaskBreakdown(
  supervisor,
  taskPlanningService,
) {
...
async function demonstrateTeamAssembly(supervisor) {
  ...
  const teamResult = await supervisor.execute({
    ...
    input: {
      taskDescription:
        'Develop a responsive navigation component for the website',
...
async function demonstrateTaskDelegation(supervisor, teamId) {
  ...
  const delegationResult = await supervisor.execute({
    ...
    input: {
      ...
      description:
        'Create a responsive navigation component with dropdown menus and mobile-friendly behavior',
...
async function demonstrateSuggestionCoordination(supervisor) {
  ...
  const suggestionResult = await supervisor.execute({
    ...
    input: {
      ...
      suggestion:
        'We should create a shared design system before implementing individual components',
      agentId: 'planning-agent-1',
      weight: 3.0,
    },
  });
...
async function runTest() {
  try {
    ...
    // Run the demos
    const plan = await demonstrateCollaborativeTaskBreakdown(
      supervisor,
      mockTaskPlanningService,
    );
