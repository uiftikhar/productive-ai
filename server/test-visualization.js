...
const { RealTimeGraphRendererImpl } = require('./src/langgraph/adaptive/visualization/dynamic-graph/real-time-graph-renderer.service');
const { DecisionCaptureImpl } = require('./src/langgraph/adaptive/visualization/agent-reasoning/decision-capture.service');
...
class MockedVisualizationClient {
  ...
  handleGraphUpdate(graph) {
    console.log(`[MockedClient] Received graph update: ${graph.name}, ${graph.nodes.length} nodes, ${graph.edges.length} edges`);
    this.updates.push({
      timestamp: new Date(),
  ...
  handleHighlightUpdate(highlights) {
    console.log(`[MockedClient] Received highlight update: ${highlights.nodeIds.length} nodes, ${highlights.edgeIds.length} edges`);
...
async function testDynamicGraphVisualization() {
  ...
  const graphId = graphRenderer.initializeGraph('workflow-123', 'Test Workflow');
  console.log(`Created graph: ${graphId}`);
  
  // Subscribe to updates
  const unsubscribe = graphRenderer.subscribeToGraphUpdates(graphId, (graph) => {
  ...
  setTimeout(() => {
    graphRenderer.updateNode(graphId, taskA.id, { state: GraphNodeState.ACTIVE });
  }, 500);
  
  setTimeout(() => {
    graphRenderer.updateNode(graphId, taskA.id, { state: GraphNodeState.COMPLETED });
    graphRenderer.updateNode(graphId, taskB.id, { state: GraphNodeState.ACTIVE });
  }, 1000);
...
async function testAgentReasoningVisualization() {
  ...
  const decision1Id = decisionCapture.recordDecisionPoint({
    ...
    reasoning: 'Data analysis first provides more reliable results with manageable time trade-off.',
  ...
  const decision2Id = decisionCapture.recordDecisionPoint({
    ...
    reasoning: 'Pattern A provides the best balance of confidence and simplicity.',
  ...
  const decision3Id = decisionCapture.recordDecisionPoint({
    ...
    reasoning: 'Results are good but additional validation will provide more confidence.',
...
async function testTeamFormationVisualization() {
  ...
  console.log('This test will be implemented when team visualization services are completed.');
  return true;
}
...
async function testInteractiveWorkflowInspector() {
  ...
  console.log('This test will be implemented when interactive workflow services are completed.');
  return true;
}

// Main test runner
async function runTests() {
  console.log('===== Testing Milestone 4: Emergent Workflow Visualization =====');
