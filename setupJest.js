...
class MockLogger {
  constructor() {
    this.logs = [];
    this.debug = jest.fn((message, context) => this.logs.push({ level: 'debug', message, context }));
    this.info = jest.fn((message, context) => this.logs.push({ level: 'info', message, context }));
    this.warn = jest.fn((message, context) => this.logs.push({ level: 'warn', message, context }));
    this.error = jest.fn((message, context) => this.logs.push({ level: 'error', message, context }));
  }
}
  ...
  return {
    PineconeConnectionService: jest.fn().mockImplementation(() => {
      ...
      return {
        ...
        deleteAllVectorsInNamespace: jest.fn().mockResolvedValue({ deletedCount: 100 }),
        describeIndexStats: jest.fn().mockResolvedValue({
          namespaces: { 'test-namespace': { vectorCount: 100 } },
          dimension: 128,
          indexFullness: 0.5,
        ...
        findSimilar: jest.fn().mockImplementation((indexName, vector, topK, namespace) => {
          return mockQueryVectors(indexName, vector, { topK, includeMetadata: true }, namespace);
        }),
        namespaceExists: jest.fn().mockImplementation((indexName, namespace) => {
        ...
        listNamespaces: jest.fn().mockImplementation((indexName) => {
          ...
          const lastCall = mockStatsFunc.mock.calls.length > 0 ? 
            mockStatsFunc.mock.results[mockStatsFunc.mock.calls.length - 1].value : 
            { namespaces: { 'namespace1': {}, 'namespace2': {} } };
          return Promise.resolve(Object.keys(lastCall.namespaces || {}));
        }),
        getNamespaceVectorCount: jest.fn().mockImplementation((indexName, namespace) => {
          if (namespace === 'test-namespace') {
        ...
        executeWithRetry: jest.fn().mockImplementation(async (fn, operationName, retries = 3) => {
          try {
            ...
            global.mockLogger.warn(`Retrying Pinecone operation ${operationName}`, {
  ...
  return {
    EmbeddingService: jest.fn().mockImplementation(() => ({
      embedText: jest.fn().mockImplementation((text) => {
        const length = 1536;
        const embedding = new Array(length).fill(0).map((_, i) => 0.1 * (i % 10));
  ...
  return {
    PineconeConfig: {
      getInstance: jest.fn().mockReturnValue({
        index: 'test-index',
        client: mockClient,
        getControllerHostUrl: jest.fn().mockReturnValue('https://test-controller.pinecone.io'),
