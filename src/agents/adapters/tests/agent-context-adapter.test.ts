...
jest.mock('../../../shared/user-context/services/conversation-context.service.ts');
...
describe('AgentContextAdapter', () => {
  ...
  beforeEach(() => {
    ...
    mockConversationService = {
      ...
      getConversationHistory: jest.fn().mockResolvedValue([
        ...
        { role: 'assistant', content: 'Hi there!', timestamp: Date.now() - 50000 }
      ]),
  ...
  test('initialize should initialize all context services', async () => {
    ...
    expect(mockLogger.info).toHaveBeenCalledWith('Initializing AgentContextAdapter');
    expect(mockLogger.info).toHaveBeenCalledWith('AgentContextAdapter initialization complete');
  });
  ...
  test('getContext should retrieve context from all available services', async () => {
    ...
    expect(mockConversationService.getConversationHistory).toHaveBeenCalledWith(userId, options.conversationId, 20);
  ...
  test('clearContext should clear context from all services when contextType is all', async () => {
    ...
    expect(mockConversationService.clearUserContext).toHaveBeenCalledWith(userId);
  ...
  test('clearContext should only clear context from specified services', async () => {
    ...
    await adapter.clearContext(userId, { contextTypes: ['conversation', 'document'] });
  ...
  test('getContext should handle service errors gracefully', async () => {
    ...
    mockDocumentService.searchDocumentContent.mockRejectedValueOnce(new Error('Service unavailable'));
    
    // Should not throw
    const result = await adapter.getContext(userId, input);
    
    // Error should be logged
    expect(mockLogger.error).toHaveBeenCalledWith('Error retrieving document context', expect.any(Object));
