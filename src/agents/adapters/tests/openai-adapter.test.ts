...
describe('OpenAIAdapter', () => {
  ...
  test('error in generateChatCompletion should be logged and rethrown', async () => {
    ...
    await expect(adapter.generateChatCompletion(messages)).rejects.toThrow('LLM API error');
    expect(mockLogger.error).toHaveBeenCalledWith('Error generating chat completion', {
      error: 'LLM API error'
    });
