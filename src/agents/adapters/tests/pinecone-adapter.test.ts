...
describe('PineconeAdapter', () => {
  ...
  beforeEach(() => {
    ...
    mockPineconeService = {
      ...
      queryVectors: jest.fn().mockResolvedValue({
        matches: [{ id: 'doc1', score: 0.9, metadata: { title: 'Test Document' } }],
  ...
  test('initialize should call through to the Pinecone service', async () => {
    ...
    expect(mockLogger.info).toHaveBeenCalledWith('Initializing PineconeAdapter');
  });
  ...
  test('storeVector should properly format and call upsertVectors', async () => {
    ...
    expect(mockLogger.debug).toHaveBeenCalledWith('Storing vector', expect.any(Object));
  });
  ...
  test('storeVectors should properly format multiple vectors and call upsertVectors', async () => {
    ...
    expect(mockLogger.debug).toHaveBeenCalledWith('Storing multiple vectors', expect.any(Object));
  });
  ...
  test('querySimilar should properly format query options and transform results', async () => {
    ...
    mockPineconeService.queryVectors.mockResolvedValueOnce({
      matches: [
        { id: 'doc1', score: 0.95, metadata: { title: 'Doc 1' }, values: [0.1, 0.2, 0.3] },
        { id: 'doc2', score: 0.85, metadata: { title: 'Doc 2' }, values: [0.4, 0.5, 0.6] },
        { id: 'doc3', score: 0.75, metadata: { title: 'Doc 3' }, values: [0.7, 0.8, 0.9] }
      ],
      namespace: 'test-namespace'
    });
  ...
  test('namespaceExists should call through to the Pinecone service', async () => {
    ...
    expect(mockPineconeService.namespaceExists).toHaveBeenCalledWith(indexName, namespace);
