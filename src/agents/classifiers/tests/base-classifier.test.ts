...
describe('BaseClassifier', () => {
  ...
  describe('agent setting', () => {
    test('should set agents and format descriptions', () => {
      ...
      expect(variables.AGENT_DESCRIPTIONS).toContain(
        'agent1: Agent 1 description',
      );
      expect(variables.AGENT_DESCRIPTIONS).toContain(
        'agent2: Agent 2 description',
      );
    });
  });
  ...
  describe('template handling', () => {
    test('should set custom prompt template', () => {
      ...
      expect(classifier['fillTemplate'](template, variables)).toBe(
        'Custom template test input',
      );
    });
  ...
  describe('classification', () => {
    ...
    test('should retry classification on error', async () => {
      ...
      expect(mockLogger.hasMessage('Classification attempt 1 failed')).toBe(
        true,
      );
      expect(mockLogger.hasMessage('Classification attempt 2 failed')).toBe(
        true,
      );
      expect(mockLogger.hasMessage('Classification attempt 3 failed')).toBe(
        true,
      );
    });
