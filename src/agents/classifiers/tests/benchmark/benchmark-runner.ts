...
async function runBenchmark() {
  ...
  const flags = {
    classifier:
      args.find((arg) => arg.startsWith('--classifier='))?.split('=')[1] ||
      'openai',
    categories:
      args
  ...
  if (flags.help) {
    ...
    console.log('Options:');
    console.log(
      '  --classifier=<name>      The classifier to benchmark (openai, bedrock) [default: openai]',
    );
    console.log(
      '  --categories=<list>      Comma-separated list of categories to include [default: all]',
    );
    console.log(
      '  --output=<path>          Write report to the specified file [default: none]',
    );
    ...
    console.log('Example:');
    console.log(
      '  ts-node benchmark-runner.ts --classifier=openai --categories=general,technical --output=report.md',
    );
    process.exit(0);
  }
