...
async function compareClassifiers() {
  ...
  for (const item of BENCHMARK_DATASET) {
    ...
    try {
      ...
      const openaiResult = await openaiClassifier.classify(item.input, conversationHistory);
      const bedrockResult = await bedrockClassifier.classify(item.input, conversationHistory);
      ...
      // Print comparison
      console.log(`Expected: ${item.expected.agentId || 'null'}, Follow-up: ${item.expected.isFollowUp}`);
      console.log(`OpenAI:   ${openaiResult.selectedAgentId || 'null'} (${openaiResult.confidence.toFixed(2)}) ${openaiIsCorrect ? '✅' : '❌'}`);
      console.log(`Bedrock:  ${bedrockResult.selectedAgentId || 'null'} (${bedrockResult.confidence.toFixed(2)}) ${bedrockIsCorrect ? '✅' : '❌'}`);
      console.log('');
  ...
  console.log(`OpenAI:  ${openaiAccuracy.toFixed(2)}% (${openaiCorrect}/${BENCHMARK_DATASET.length})`);
  console.log(`Bedrock: ${bedrockAccuracy.toFixed(2)}% (${bedrockCorrect}/${BENCHMARK_DATASET.length})`);
  console.log('');
  ...
  for (const category of categories) {
    const categoryItems = BENCHMARK_DATASET.filter(item => item.category === category);
    const categoryResults = results.filter(r => 
      BENCHMARK_DATASET.find(item => item.input === r.input)?.category === category
    );
    
    const openaiCategoryCorrect = categoryResults.filter(r => r.openai.isCorrect).length;
    const bedrockCategoryCorrect = categoryResults.filter(r => r.bedrock.isCorrect).length;
    
    const openaiCategoryAccuracy = (openaiCategoryCorrect / categoryItems.length) * 100;
    const bedrockCategoryAccuracy = (bedrockCategoryCorrect / categoryItems.length) * 100;
    
    console.log(`${category}:`);
    console.log(`  OpenAI:  ${openaiCategoryAccuracy.toFixed(2)}% (${openaiCategoryCorrect}/${categoryItems.length})`);
    console.log(`  Bedrock: ${bedrockCategoryAccuracy.toFixed(2)}% (${bedrockCategoryCorrect}/${categoryItems.length})`);
  }
