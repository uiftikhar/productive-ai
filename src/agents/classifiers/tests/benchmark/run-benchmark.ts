...
async function runBenchmark(classifierType: 'openai' | 'bedrock') {
  ...
  for (const item of BENCHMARK_DATASET) {
    try {
      ...
      if (isCorrect) {
        correct++;
        console.log(
          `✅ Correct: ${result.selectedAgentId}, confidence: ${result.confidence.toFixed(2)}`,
        );
      } else {
        console.log(
          `❌ Incorrect: Expected ${item.expected.agentId}, got ${result.selectedAgentId}`,
        );
        console.log(
          `   Confidence: ${result.confidence.toFixed(2)}, isFollowUp: ${result.isFollowUp}`,
        );
      ...
      console.log(
        `❌ Error testing "${item.input.substring(0, 30)}...": ${error instanceof Error ? error.message : String(error)}`,
      );
      results.push({
        item,
        result: null,
  ...
  console.log(
    `Accuracy: ${accuracy.toFixed(2)}% (${correct}/${BENCHMARK_DATASET.length})\n`,
  );
  ...
  for (const category of categories) {
    ...
    console.log(
      `${category}: ${categoryAccuracy.toFixed(2)}% (${categoryCorrect}/${categoryItems.length})`,
    );
  }
...
if (require.main === module) {
  ...
  runBenchmark(classifierType).catch((error) => {
