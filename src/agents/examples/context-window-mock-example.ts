...
class MockConversationStorage {
  ...
  async deleteConversation(userId: string, conversationId: string): Promise<number> {
    const count = this.messages.filter(m => 
      m.userId === userId && m.conversationId === conversationId
    ).length;
  ...
  async getSegments(userId: string, conversationId: string): Promise<{
    segmentId: string;
    segmentTopic?: string;
    turnCount: number;
    ...
    return Array.from(segments.values())
      .map((segment) => ({
        ...
        agentIds: segment.agentIds.size > 0 ? Array.from(segment.agentIds) : undefined,
      }))
...
const mockEmbeddingService = {
  generateEmbedding: async (text: string): Promise<number[]> => {
    ...
    return Array(128).fill(0).map((_, i) => rng(i) * 2 - 1);
  }
};
...
class ContextWindowService {
  ...
  async getCurrentSegmentId(userId: string, conversationId: string): Promise<string> {
    const messages = await this.storage.getMessages(userId, conversationId, { limit: 1 });
  ...
  async getConversationHistory(
    ...
    if (options.sortBy === 'relevance' && options.relevanceEmbedding) {
      return this.storage.getMessagesByRelevance(userId, conversationId, options.relevanceEmbedding, {
        limit,
        segmentId: options.segmentId,
        agentId: options.agentId
      });
  ...
  async createContextWindow(
    ...
    if (options.relevanceQuery || options.relevanceEmbedding) {
      const queryEmbedding = options.relevanceEmbedding || 
        await mockEmbeddingService.generateEmbedding(options.relevanceQuery || '');
      
      messages = await this.storage.getMessagesByRelevance(userId, conversationId, queryEmbedding, {
    ...
    if (options.includeAgentIds && options.includeAgentIds.length > 0) {
      messages = messages.filter(message => {
        return message.agentId && options.includeAgentIds?.includes(message.agentId);
      });
    }
    
    if (options.excludeAgentIds && options.excludeAgentIds.length > 0) {
      messages = messages.filter(message => {
        return !message.agentId || !options.excludeAgentIds?.includes(message.agentId);
      });
    }
    ...
    if (options.filterByCapabilities && options.filterByCapabilities.length > 0) {
      messages = messages.filter(message => {
        return message.capability && options.filterByCapabilities?.includes(message.capability);
      });
    }
...
async function runExample() {
  try {
    ...
    // Clean up test data
    const deletedTurns = await contextService.deleteConversation(userId, conversationId);
...
async function demonstrateContextWindows(
  ...
  for (const message of technicalContext.messages) {
    console.log(`- ${message.role}: "${message.message.substring(0, 50)}..." (Agent: ${message.agentId || 'User'})`);
  }
  ...
  console.log(`Retrieved ${warrantyMessages.length} messages from warranty segment`);
  const segments = await contextService.getConversationSegments(userId, conversationId);
  const warrantySegment = segments.find(s => s.segmentId === 'segment-warranty');
  ...
  for (const message of capabilityContext.messages) {
    console.log(`- ${message.role}: "${message.message.substring(0, 50)}..." (Capability: ${message.capability || 'N/A'})`);
  }
  ...
  for (const message of relevanceContext.messages) {
    console.log(`- ${message.role}: "${message.message.substring(0, 50)}..." (Score: ${message.score?.toFixed(3) || 'N/A'})`);
  }
