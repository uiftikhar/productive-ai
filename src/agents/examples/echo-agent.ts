...
export class EchoAgent extends BaseAgent {
  ...
  constructor(
    ...
    super(
      ...
      options.description ||
        'Repeats input back to the user with optional enhancement',
      {
  ...
  protected async executeInternal(
        ...
        const messages: MessageConfig[] = [
          {
            role: 'system',
            content:
              "You are an echo assistant. Your job is to enhance the user's message by adding some personality and humor, but keep it focused on the original message.",
          },
          {
            role: 'user',
            content: `Enhance this message while keeping its meaning: "${responseText}"`,
          },
        ];
    ...
    if (
      request.context &&
      Object.keys(request.context.metadata || {}).length > 0
    ) {
