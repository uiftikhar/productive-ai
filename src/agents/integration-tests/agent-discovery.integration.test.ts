...
describe('AgentDiscoveryService Integration Tests', () => {
  ...
  const sampleAgents = [
    {
      ...
      execute: jest.fn(),
      canHandle: (capability: string) => ['retrieve_knowledge', 'process_context'].includes(capability)
    },
    {
      ...
      execute: jest.fn(),
      canHandle: (capability: string) => ['retrieve_knowledge', 'process_code'].includes(capability)
    },
    {
      ...
      execute: jest.fn(),
      canHandle: (capability: string) => ['retrieve_knowledge', 'verify_facts'].includes(capability)
    },
    {
      id: 'response-agent-1',
      ...
      execute: jest.fn(),
      canHandle: (capability: string) => ['generate_response', 'format_output'].includes(capability)
    },
    {
      id: 'response-agent-2',
      ...
      execute: jest.fn(),
      canHandle: (capability: string) => ['generate_response', 'creative_writing'].includes(capability)
    },
    {
      ...
      execute: jest.fn(),
      canHandle: (capability: string) => ['orchestrate_workflow', 'route_tasks'].includes(capability)
    }
  ];
  ...
  beforeEach(() => {
    ...
    registry = {
      listAgents: jest.fn().mockReturnValue(sampleAgents as unknown as AgentInterface[]),
      getAgent: jest.fn().mockImplementation((id: string) => 
        sampleAgents.find(agent => agent.id === id) as unknown as AgentInterface),
      findAgentsWithCapability: jest.fn().mockImplementation((capability: string) => 
        sampleAgents.filter(agent => agent.capabilities.includes(capability)) as unknown as AgentInterface[]),
  ...
  test('Should discover agent by capability with balanced weights', () => {
    ...
    expect(registry.findAgentsWithCapability).toHaveBeenCalledWith('retrieve_knowledge');
    
    // Specific agent selected should be one with the capability
    const selectedAgent = sampleAgents.find(agent => agent.id === result?.agentId);
  ...
  test('Should prioritize performance when weight is higher', () => {
    ...
    const knowledgeAgents = sampleAgents.filter(
      agent => agent.capabilities.includes('retrieve_knowledge')
    );
    
    const fastestKnowledgeAgentId = [...agentPerformanceMetrics.entries()]
      .filter(([id]) => knowledgeAgents.some(agent => agent.id === id))
      .sort((a, b) => a[1].averageExecutionTimeMs - b[1].averageExecutionTimeMs)[0][0];
  ...
  test('Should prioritize reliability when weight is higher', () => {
    ...
    for (const agent of sampleAgents) {
      if (agent.capabilities.includes('retrieve_knowledge')) {
        ...
        const metrics = discoveryService.getAgentMetrics(agent.id, 'retrieve_knowledge');
      }
    }
  ...
  test('Should return null if all matching agents are excluded', () => {
    ...
    const result = discoveryService.discoverAgent({
      capability: 'retrieve_knowledge',
      excludedAgentIds: ['knowledge-agent-1', 'knowledge-agent-2', 'knowledge-agent-3']
    });
  ...
  test('Should handle multiple capability requirements', () => {
    ...
    const agentsWithProcessCode = sampleAgents.filter(agent => 
    ...
    discoveryService.updateAgentMetrics('knowledge-agent-2', 'retrieve_knowledge', {
  ...
  test('Should update agent metrics', () => {
    ...
    discoveryService.updateAgentMetrics('knowledge-agent-1', 'retrieve_knowledge', {
      executionTime,
      success
    });
