...
describe('ModelRouterService Integration Tests', () => {
  ...
  const sampleContextItems = [
    ...
    },
    {
      id: 'context-3',
      content:
        'Project Alpha milestone 3: Development phase completion (June 15)',
      metadata: {
        source: 'project-plan',
        relevance: 0.91,
      },
    ...
    },
    {
      id: 'context-5',
      content:
        'Project Alpha milestone 5: Deployment to production (September 15)',
      metadata: {
        source: 'project-plan',
        relevance: 0.87,
      },
  ...
  beforeEach(() => {
    ...
    ragPromptManager.formatContextItems = jest
      .fn()
  ...
  test('Should manage context window based on available space', async () => {
    ...
    expect(managedMediumContext.length).toBeGreaterThanOrEqual(
      managedSmallContext.length,
    );
  ...
  test('Should process streaming requests correctly', async () => {
    ...
    const originalProcessRequest = modelRouter.processRequest;
    // @ts-ignore: Mock method for testing
    modelRouter.processRequest = jest
      .fn()
      .mockImplementation(
        (
  ...
  test('Should handle tokenization for different model families', async () => {
    ...
    try {
      ...
      const tokenCountDifference = Math.abs(
        openaiTokenCount - claudeTokenCount,
      );
