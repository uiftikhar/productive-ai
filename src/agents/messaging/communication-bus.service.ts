...
import { AgentMessage, MessagePriority, MessageType } from './agent-message.interface.ts';
...
export class CommunicationBusService {
   ...
   */
  public async publish(message: Omit<AgentMessage, 'id' | 'timestamp'>): Promise<string> {
    ...
    if (completeMessage.targetId) {
      this.eventEmitter.emit(`message:target:${completeMessage.targetId}`, completeMessage);
    }
    
    // If the message has a topic, emit a topic-specific event
    if (completeMessage.topic) {
      this.eventEmitter.emit(`message:topic:${completeMessage.topic}`, completeMessage);
    }
    
    // If the message has a type, emit a type-specific event
    if (completeMessage.type) {
      this.eventEmitter.emit(`message:type:${completeMessage.type}`, completeMessage);
    }
   ...
   */
  public subscribe(options: MessageSubscriptionOptions, handler: MessageHandler): string {
    ...
    if (options.agentId) {
      ...
      this.eventEmitter.on(`message:type:${options.messageType}`, messageHandler);
    } else {
