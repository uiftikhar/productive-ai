  ...
  return {
    ...
    StreamingResponseManager: {
      getInstance: jest.fn().mockImplementation(() => ({
        createStreamingHandler: jest.fn().mockImplementation((streamId, callbacks, options) => {
...
describe('MultiAgentStreamingAggregator', () => {
  ...
  describe('Basic functionality', () => {
    ...
    test('should complete aggregation when all streams are complete', () => {
      ...
      const handler1 = aggregator.registerAgentStream(metadata1, options) as TestStreamingHandler;
      const handler2 = aggregator.registerAgentStream(metadata2, options) as TestStreamingHandler;
      
      // Define listener properties for our handlers
      Object.defineProperty(handler1, 'listener', {
        value: {
          onToken: (token: string) => aggregator['handleAgentToken'](handler1.streamId, token),
          onComplete: (response: string) => aggregator['handleAgentComplete'](handler1.streamId, response),
          onError: (error: Error) => aggregator['handleAgentError'](handler1.streamId, error)
        }
      });
      
      Object.defineProperty(handler2, 'listener', {
        value: {
          onToken: (token: string) => aggregator['handleAgentToken'](handler2.streamId, token),
          onComplete: (response: string) => aggregator['handleAgentComplete'](handler2.streamId, response),
          onError: (error: Error) => aggregator['handleAgentError'](handler2.streamId, error)
        }
      });
    ...
    test('should handle errors in streams', () => {
      ...
      Object.defineProperty(handler, 'listener', {
        value: {
          onToken: (token: string) => aggregator['handleAgentToken'](handler.streamId, token),
          onComplete: (response: string) => aggregator['handleAgentComplete'](handler.streamId, response),
          onError: (error: Error) => aggregator['handleAgentError'](handler.streamId, error)
        }
      });
      ...
      const cancelAggregator = new MultiAgentStreamingAggregator(cancelCallback);
  ...
  describe('Aggregation strategies', () => {
    test('should correctly aggregate streams in PARALLEL mode', () => {
      ...
      const expectedOutput = '**Test Agent 1**: Response from agent 1\n\n**Test Agent 2**: Response from agent 2';
      expect(mockCallback.onComplete).toHaveBeenCalledWith(
        expectedOutput,
        expect.any(Object)
      );
    ...
    test('should correctly aggregate streams in LEADER_FOLLOWER mode', () => {
      ...
      expect(mockCallback.onComplete).toHaveBeenCalledWith(
        expect.stringContaining('**Leader Agent**: Main response from the leader'),
        expect.any(Object)
      );
