import { AgentTaskExecutorService, TaskExecutionOptions, TaskExecutionEvent, TaskExecutionEventType } from '../agent-task-executor.service';
...
describe('AgentTaskExecutorService', () => {
  ...
  beforeEach(() => {
    ...
    mockPlanningService = {
      ...
      updateTaskStatus: jest.fn().mockImplementation((planId, taskId, status, result, failureReason, agentId) => {
        return true;
      }),
    ...
    jest.spyOn(EventEmitter.prototype, 'emit').mockImplementation(function(this: EventEmitter, eventName: any, ...args: any[]) {
    ...
    jest.spyOn(taskExecutorService as any, 'unsubscribe').mockImplementation((subId) => {
      // Remove all listeners for the actual test
      (taskExecutorService as any).eventEmitter.removeAllListeners('task-event');
    });
  });
  ...
  describe('executeTaskDirectly', () => {
    it('should execute a task with an assigned agent', async () => {
      const result = await taskExecutorService.executeTaskDirectly('plan-123', 'task-123');
    ...
    it('should throw an error if task is not found', async () => {
      ...
      jest.spyOn(taskExecutorService as any, 'executeTaskDirectly').mockImplementationOnce(async (planId, taskId) => {
        throw new Error('Task not found');
      });

      await expect(taskExecutorService.executeTaskDirectly('plan-123', 'non-existent-task')).rejects.toThrow('Task not found');
    });
    ...
    it('should throw an error if agent is not found', async () => {
      ...
      await expect(taskExecutorService.executeTaskDirectly('plan-123', 'task-123')).rejects.toThrow('Agent not found');
    });
  });
  ...
  describe('executePlan', () => {
    it('should execute all ready tasks in a plan', async () => {
      ...
      mockPlanningService.getReadyTasks.mockReturnValue([readyTask1, readyTask2]);

      // Spy on private executeTask method
      const executeTaskSpy = jest.spyOn(taskExecutorService as any, 'executeTask');
      executeTaskSpy.mockImplementation(
        async (plan, task) => {
          return {
      ...
      expect(mockPlanningService.getReadyTasks).toHaveBeenCalledWith('plan-123');
  ...
  describe('event subscription', () => {
    it('should allow subscribing to specific event types', async () => {
      ...
      try {
        ...
        const originalExecuteTaskDirectly = taskExecutorService.executeTaskDirectly;
        jest.spyOn(taskExecutorService, 'executeTaskDirectly').mockImplementation(
          async (planId, taskId) => {
        ...
        expect(specificEventHandler).toHaveBeenCalledWith(expect.objectContaining({
          type: TaskExecutionEventType.TASK_COMPLETED,
        }));
      } finally {
