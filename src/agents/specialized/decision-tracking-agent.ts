...
import { SystemRole, SystemRoleEnum } from '../../shared/prompts/prompt-types.ts';
import { InstructionTemplateName, InstructionTemplateNameEnum } from '../../shared/prompts/instruction-templates.ts';
...
} from './types/decision-tracking.types.ts';
...
import { InjectLogger } from '../../shared/logger/inject-logger.decorator.ts';
...
export class DecisionTrackingAgent extends BaseAgent {
  ...
  constructor(
    ...
    if (options.embeddingService) {
      ...
      this.embeddingService = new EmbeddingService(this.openaiAdapter, this.logger);
    } else {
    ...
    this.registerCapability({
      ...
      parameters: {
        meetingAnalysis: 'The analysis of a meeting containing transcript and context',
        identifyOnly: 'Only identify decisions without categorizing or analyzing them',
    ...
    this.registerCapability({
      ...
      parameters: {
        format: 'Report format (summary, detailed, timeline, impact, dashboard)',
    ...
    this.registerCapability('generateDecisionReport', {
      ...
      parameters: {
        ...
        timeframe: {
          type: 'string',
          required: false,
          description: 'Timeframe for decisions to include (e.g., "last_week", "last_month")'
        },
        confidenceThreshold: {
          type: 'number',
          required: false,
          description: 'Minimum confidence threshold for decisions to include (0-100)'
        },
  ...
  private registerMessageHandler(): void {
    ...
    this.communicationChannel.registerHandler(async (message: AgentCommunicationMessage) => {
    ...
    try {
      switch (capability) {
        case 'analyze_decisions': {
          ...
          const meetingAnalysis = typeof request.input === 'string' 
        ...
        case 'trackDecisions': {
          const { meetingId, transcript, userId } = request.parameters as { meetingId: string; transcript: string; userId: string };
          
          await this.trackDecisions(meetingId, transcript, userId);
          
          return {
            output: JSON.stringify({ message: 'Decisions tracked successfully' }),
          };
        }
        
        case 'generateDecisionReport': {
          const { userId, timeframe, confidenceThreshold, performImpactAssessment } = request.parameters as { userId: string; timeframe: string; confidenceThreshold: number; performImpactAssessment: boolean };
          
          const report = await this.generateDecisionReport(userId, timeframe, confidenceThreshold, performImpactAssessment);
  ...
  async analyzeDecisions(
    ...
    this.logger.info(`Analyzing decisions for meeting ${meetingAnalysis.meetingId}`);
    ...
    try {
      ...
      if (meetingAnalysis.decisions && meetingAnalysis.decisions.length > 0) {
        decisions = meetingAnalysis.decisions.map(d => this.convertMeetingDecisionToTrackingDecision(d));
      }
   ...
   */
  private async identifyDecisions(meetingAnalysis: MeetingAnalysis): Promise<Decision[]> {
  ...
  async categorizeDecisions(decisions: Decision[]): Promise<Decision[]> {
    ...
    try {
      ...
      // Parse the response
      const parsedResult = this.parseJsonResponse<{ category: string; impact: string; explanation: string }[]>(responseText);
  ...
  async assessImpact(decisions: Decision[]): Promise<Decision[]> {
    ...
    try {
      ...
      // Parse the response
      const parsedResult = this.parseJsonResponse<{ impact: string; explanation: string }[]>(responseText);
  ...
  async trackDependencies(decisions: Decision[]): Promise<Decision[]> {
    ...
    try {
      ...
      // Parse the response
      const parsedResult = this.parseJsonResponse<{ id: string; dependencies: string[] }[]>(responseText);
    ...
    try {
      ...
      if (params.status && params.status.length > 0) {
        filteredDecisions = filteredDecisions.filter(
          decision => params.status?.includes(decision.status as DecisionStatus)
        );
      }
      ...
      if (params.categories && params.categories.length > 0) {
        filteredDecisions = filteredDecisions.filter(
          decision => params.categories?.includes(decision.category as DecisionCategory)
        );
      }
      ...
      if (params.impact && params.impact.length > 0) {
        filteredDecisions = filteredDecisions.filter(
          decision => params.impact?.includes(decision.impact as 'low' | 'medium' | 'high')
        );
      }
      ...
      if (params.decisionMakers && params.decisionMakers.length > 0) {
        filteredDecisions = filteredDecisions.filter(
          decision => decision.decisionMaker && params.decisionMakers?.includes(decision.decisionMaker)
        );
      }
      ...
      if (params.tags && params.tags.length > 0) {
        filteredDecisions = filteredDecisions.filter(
          decision => decision.tags && params.tags?.some(tag => decision.tags?.includes(tag))
        );
      }
      ...
      if (params.meetingIds && params.meetingIds.length > 0) {
        filteredDecisions = filteredDecisions.filter(
          decision => params.meetingIds?.includes(decision.source.meetingId)
        );
      }
      ...
      if (params.searchText) {
        ...
        filteredDecisions = filteredDecisions.filter(
          decision => 
            ...
            (decision.summary && decision.summary.toLowerCase().includes(searchText))
        );
      }
   ...
   */
  private calculateReportSummary(decisions: Decision[]): DecisionReport['summary'] {
    // Count by status
  ...
  private async generateInsights(
    ...
    const response = await this.openaiAdapter.generateChatCompletion(messageConfig);
  ...
  async trackDecisions(meetingId: string, transcript: string, userId: string): Promise<void> {
    // Implementation of trackDecisions method
  }

  async generateDecisionReport(userId: string, timeframe: string, confidenceThreshold: number, performImpactAssessment: boolean): Promise<string> {
