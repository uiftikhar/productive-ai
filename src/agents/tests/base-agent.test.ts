...
class TestAgent extends BaseAgent {
  ...
  constructor() {
    super('Test Agent', 'Agent for testing BaseAgent functionality', {
  ...
  protected async executeInternal(
    ...
    // Simulate an error if testError is true
    if (this.testError) {
    ...
    return {
      output: `Processed: ${typeof request.input === 'string' ? request.input : 'complex input'}`,
    };
  }
}
...
describe('BaseAgent', () => {
  ...
  test('should initialize correctly', async () => {
    expect(agent.getState().status).toBe(AgentStatus.INITIALIZING);
  ...
  test('should have correct agent properties', () => {
    ...
    expect(agent.description).toBe('Agent for testing BaseAgent functionality');
  });

  test('should register and check capabilities', () => {
