...
`;
  ...
  beforeEach(() => {
    ...
    (
      OpenAIConnector as jest.MockedClass<typeof OpenAIConnector>
    ).mockImplementation(() => mockOpenAIConnector);

    // Mock agent methods
    jest
      .spyOn(DecisionTrackingAgent.prototype as any, 'identifyDecisions')
      .mockImplementation(async () => mockDecisions);

    jest
      .spyOn(DecisionTrackingAgent.prototype as any, 'trackDecisions')
      .mockImplementation(async () => mockDecisions);

    jest
      .spyOn(DecisionTrackingAgent.prototype as any, 'generateDecisionReport')
      .mockImplementation(async () => mockDecisionReport);

    jest
      .spyOn(DecisionTrackingAgent.prototype as any, 'analyzeDecisionImpact')
  ...
  test('should initialize successfully', async () => {
    ...
    expect(
      global.mockLogger.hasMessage(
        'DecisionTrackingAgent initialized successfully',
        'info',
      ),
    ).toBe(true);
  });
  ...
  test('should handle errors gracefully', async () => {
    ...
    expect(
      global.mockLogger.hasMessage(
        'Error executing DecisionTrackingAgent',
        'error',
      ),
    ).toBe(true);
  });
