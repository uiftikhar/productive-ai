...
jest.mock('../../../shared/services/user-context/document-context.service');
jest.mock('../../../shared/services/user-context/conversation-context.service');
jest.mock('../../../shared/services/user-context/meeting-context.service');
jest.mock(
  '../../../shared/services/user-context/relevance-calculation.service',
);
...
describe('KnowledgeRetrievalAgent Integration', () => {
  ...
  beforeEach(() => {
    ...
    mockMeetingContextService = {
      findUnansweredQuestions: jest
        .fn()
        .mockResolvedValue(sampleMeetingContext),
      initialize: jest.fn().mockResolvedValue(undefined),
    ...
    (EmbeddingServiceFactory.getService as jest.Mock).mockReturnValue(
      mockEmbeddingService,
    );
    ...
    agent = new KnowledgeRetrievalAgent({
      ...
      llm: {
        invoke: jest.fn().mockResolvedValue({
          content:
            'AI is a field of computer science that focuses on creating machines that can perform tasks typically requiring human intelligence.',
        }),
  ...
  test('should initialize successfully', async () => {
    ...
    expect(
      global.mockLogger.hasMessage(
        'Knowledge Retrieval Agent initialized',
        'info',
      ),
    ).toBe(true);
  });
  ...
  test('should register the correct capabilities', () => {
    ...
    const retrieveCapability = capabilities.find(
      (c) => c.name === 'retrieve_knowledge',
    );
    const answerCapability = capabilities.find(
      (c) => c.name === 'answer_with_context',
    );
  ...
  test('should retrieve knowledge from user context', async () => {
    ...
    expect(
      mockConversationContextService.searchConversations,
    ).toHaveBeenCalled();

    // Verify meeting context service was queried
    expect(
      mockMeetingContextService.findUnansweredQuestions,
    ).toHaveBeenCalled();
  ...
  test('should pass optional parameters correctly for knowledge retrieval', async () => {
    ...
    const searchCallArgs = mockDocumentContextService.searchDocumentContent.mock
      .calls[0][2] as any;
