...
jest.mock('../../../shared/services/user-context/base-context.service');
  ...
  ChatOpenAI: jest.fn().mockImplementation(() => ({
    invoke: jest.fn().mockResolvedValue({
      content: JSON.stringify({
        ...
        decisions: [{ description: 'Delay product launch by two weeks' }],
        topics: [
          {
            title: 'Release timeline',
            participants: ['Mary', 'John', 'David'],
          },
...
John: Yes, let's do that in our next meeting. Let's wrap up for today.
  ...
  beforeEach(() => {
    ...
    (
      RagPromptManager as jest.MockedClass<typeof RagPromptManager>
    ).mockImplementation(() => mockRagPromptManager);
  ...
  test('should initialize successfully', async () => {
    ...
    expect(
      global.mockLogger.hasMessage(
        'Initializing Test Meeting Analysis Agent',
        'info',
      ),
    ).toBe(true);
  });
  ...
  test('should extract action items', async () => {
    ...
    jest
      .spyOn(agent as any, 'storeAnalysisInContext')
  ...
  test('should extract decisions', async () => {
    ...
    jest
      .spyOn(agent as any, 'extractSpecificInformation')
      .mockResolvedValue(
  ...
  test('should handle errors gracefully', async () => {
    ...
    (agent as any).llm.invoke = jest
      .fn()
      .mockRejectedValue(new Error('Test LLM failure'));
