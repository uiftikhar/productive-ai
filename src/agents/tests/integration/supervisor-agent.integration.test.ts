...
describe('SupervisorAgent Integration Tests', () => {
  ...
  beforeEach(async () => {
    ...
    supervisorAgent = new SupervisorAgent({
      ...
      logger,
      defaultTeamMembers: [
        { agent: knowledgeAgent, role: 'Researcher', priority: 8, active: true },
        { agent: decisionAgent, role: 'Decision Maker', priority: 7, active: true },
  ...
  it('should execute tasks in parallel when specified', async () => {
    ...
    jest.spyOn(knowledgeAgent, 'execute').mockImplementation(async (request) => {
      await new Promise(resolve => setTimeout(resolve, delayTimes.research));
      return { output: 'Research completed' };
    });
    
    jest.spyOn(contentAgent, 'execute').mockImplementation(async (request) => {
      await new Promise(resolve => setTimeout(resolve, delayTimes['content-creation']));
      return { output: 'Content created' };
    });
    
    jest.spyOn(decisionAgent, 'execute').mockImplementation(async (request) => {
      await new Promise(resolve => setTimeout(resolve, delayTimes['decision-tracking']));
    ...
    const sumOfDelays = Object.values(delayTimes).reduce((sum, time) => sum + time, 0);
    ...
    jest.spyOn(knowledgeAgent, 'execute').mockImplementation(
      originalKnowledgeExecute.bind(knowledgeAgent)
    );
    jest.spyOn(contentAgent, 'execute').mockImplementation(
      originalContentExecute.bind(contentAgent)
    );
    jest.spyOn(decisionAgent, 'execute').mockImplementation(
      originalDecisionExecute.bind(decisionAgent)
    );
  }, 10000);
  
  // Test error recovery
  it('should recover from agent failures and complete the workflow', async () => {
    // Create a temporary failing version of the knowledge agent
    const originalKnowledgeExecute = knowledgeAgent.execute;
    let failCount = 0;
    
    // Mock with failure for first 2 attempts
    jest.spyOn(knowledgeAgent, 'execute').mockImplementation(async (request) => {
      if (failCount < 2) {
        failCount++;
      ...
      return { output: 'Research completed after retry' };
    });
    ...
    const result = await supervisorAdapter.executeCoordinatedTask(
      ...
      [
        ...
        {
          description: 'Create content after research',
          requiredCapabilities: ['content-creation'],
          priority: 5
        }
      ],
    ...
    // Verify execution with recovery
    expect(result).toBeDefined();
    expect(result.output).toBeDefined();
    
    // Verify retry occurred
    expect(failCount).toBe(2);
    expect(knowledgeAgent.execute).toHaveBeenCalledTimes(3); // 2 fails + 1 success
    expect(contentAgent.execute).toHaveBeenCalled();
  ...
  it('should handle an overloaded system by managing execution limits', async () => {
    ...
    const manyTasks = Array(20).fill(0).map((_, i) => ({
