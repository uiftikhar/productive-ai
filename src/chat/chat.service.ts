...
export class ChatService {
  ...
  async sendMessage(request: SendMessageRequest): Promise<MessageGenerationResult> {
    try {
      ...
      this.updateUserPresence(session.userId, UserStatus.ONLINE, request.sessionId);
      ...
      try {
        ...
        if (!responseEmbeddings || !responseEmbeddings.length) {
          // Ensure we have at least one value in the embeddings
          this.logger.warn('Empty embeddings generated for response, using fallback value', {
            sessionId: request.sessionId
          });
  ...
  async updateSessionContext(
    ...
    if (options.persist) {
      try {
        ...
        const embeddings = await this.llmConnector.generateEmbedding(contextString);
  ...
  async getContextWindow(
    ...
    try {
      ...
      if (options.relevanceQuery) {
        relevanceEmbedding = await this.llmConnector.generateEmbedding(options.relevanceQuery);
      }
      ...
      const messages = await this.userContextFacade.getConversationHistory(
        ...
        options.windowSize || this.defaultHistoryLimit,
        {
          ...
          agentId: options.includeAgentIds?.length === 1 ? options.includeAgentIds[0] : undefined,
        }
      );
      ...
      const filteredMessages = messages.filter(message => {
        ...
        if (options.includeAgentIds && options.includeAgentIds.length > 0) {
          if (message.metadata?.agentId && !options.includeAgentIds.includes(message.metadata.agentId)) {
            return false;
          }
        }
        
        if (options.excludeAgentIds && options.excludeAgentIds.length > 0) {
          if (message.metadata?.agentId && options.excludeAgentIds.includes(message.metadata.agentId)) {
            return false;
          }
        }
      ...
      if (options.includeCurrentSegmentOnly) {
        ...
        if (segments.length > 0) {
          ...
          const latestSegment = segments.sort((a, b) => b.lastTimestamp - a.lastTimestamp)[0];
          segmentInfo = {
  ...
  async attachFile(
    ...
    try {
      ...
      const fileAttachments = (session.metadata?.fileAttachments || []) as any[];
      ...
      const contentTokens = textContent ? Math.ceil(textContent.length / 4) : undefined;
  ...
  async searchConversations(
    ...
    try {
      ...
      if (options.includeSessionData && mappedResults.length > 0) {
        ...
        const conversationIds = [...new Set(mappedResults.map(r => r.conversationId))];
  ...
  async findSimilarMessages(
    ...
    try {
      ...
      const embeddings = await this.llmConnector.generateEmbedding(targetMessage.content);
  ...
  async generateAnalytics(
    ...
    try {
      ...
      if (options.conversationId) {
        const conversations = await this.userContextFacade.listUserConversations(userId);
        const exists = conversations.some(c => c.conversationId === options.conversationId);
      ...
      let result: any = {
        ...
        timestamp: Date.now(),
        timeframe: {
          start: options.timeframe?.startTime || Date.now() - 30 * 24 * 60 * 60 * 1000, // Default to last 30 days
          end: options.timeframe?.endTime || Date.now()
        }
      };
      
      // Get user's conversations
      const allConversations = await this.userContextFacade.listUserConversations(userId);
      ...
      if (options.includeUsageStatistics !== false) {
        ...
        const totalTurns = conversations.reduce((sum, conv) => sum + conv.turnCount, 0);
        
        result.usageStatistics = {
          conversationCount: totalConversations,
          totalMessageCount: totalTurns,
          averageTurnsPerConversation: totalConversations > 0 
      ...
      if (options.includeSegmentAnalytics !== false && conversations.length > 0) {
        ...
        for (const conv of conversations) {
          if (conv.segments && conv.segments > 0) {
            try {
              ...
              this.logger.warn(`Failed to get segments for conversation ${conv.conversationId}`, {
                error: error instanceof Error ? error.message : String(error),
                userId
              });
            }
      ...
      this.logger.info('Generated conversation analytics', {
        ...
        resultSections: Object.keys(result).filter(k => k !== 'userId' && k !== 'timestamp' && k !== 'timeframe')
      });
  ...
  async getSessionMetrics(sessionId: string): Promise<any> {
    ...
    try {
      ...
      for (let i = 0; i < userMessages.length; i++) {
        ...
        if (assistantMsg) {
          const responseTime = assistantMsg.timestamp.getTime() - userMsg.timestamp.getTime();
      ...
      return {
        ...
        sessionDurationMs: history.length > 0
  ...
  createDiagnosticSnapshot(): Record<string, any> {
    try {
      ...
      // Get session status
      const sessionStatus = Array.from(this.sessions.entries()).map(([id, session]) => ({
        sessionId: id,
        userId: session.userId,
        conversationId: session.conversationId,
        createdAt: session.createdAt,
      ...
      // Get user presence
      const userStatus = Array.from(this.userPresence.entries()).map(([id, presence]) => ({
        userId: id,
