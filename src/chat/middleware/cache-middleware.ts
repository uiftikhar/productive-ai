...
export function createCacheMiddleware(options: CacheMiddlewareOptions = {}) {
  ...
  return function cacheMiddleware(req: Request, res: Response, next: NextFunction) {
    ...
    const userId = req.user ? ((req.user as any).id || (req.user as any).userId) : 'anonymous';
    ...
    res.send = function(body: any): Response {
      ...
      if (cacheableStatusCodes.includes(res.statusCode)) {
        ...
        headersToCache.forEach(header => {
          ...
          if (value) {
            responseToCache.headers[header] = value as string | number | string[];
          }
        });
