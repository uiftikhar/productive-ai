...
export async function testFixedHistoryAwareSupervisor(): Promise<void> {
  ...
  try {
    ...
    logger.info(
      `History verification: Retrieved ${history.length} conversation turns`,
      {
        stored: storedMessages.length,
        firstTurn:
          history.length > 0 && history[0]?.content
    ...
    if (history.length === 0) {
      ...
      logger.info(
        `Direct query results: ${queryResult.matches?.length || 0} matches`,
        {
          firstMatch:
            queryResult.matches && queryResult.matches.length > 0
              ? JSON.stringify(queryResult.matches[0].metadata).substring(
                  0,
                  100,
                ) + '...'
              : 'No matches',
      ...
      logger.info(
        `Minimal filter query results: ${minimalQueryResult.matches?.length || 0} matches`,
      );
      ...
      try {
        ...
        logger.info('Test record query results', {
          ...
          record: testQuery.matches?.[0]
            ? JSON.stringify(testQuery.matches[0].metadata).substring(0, 200)
            : 'Not found',
        });
      } catch (error) {
        logger.error('Error in direct test', {
      ...
      try {
        ...
        logger.info(
          `userId filter query results: ${userIdQueryResult.matches?.length || 0} matches`,
        );
      } catch (error) {
        logger.warn('Error with userId filter query', {
          error: error instanceof Error ? error.message : String(error),
        });
      }
      ...
      try {
        ...
        logger.info('Index stats:', {
          ...
          namespaceStats:
            JSON.stringify(stats.namespaces || {}).substring(0, 500) + '...',
        });
      } catch (error) {
        logger.warn('Error getting index stats', {
          error: error instanceof Error ? error.message : String(error),
        });
      }
      ...
      const filterVariations = [
        ...
        {
          label: 'by turnId of first message',
          filter: { turnId: storedMessages[0].turnId },
        },
        { label: 'by role=user', filter: { role: 'user' } },
        { label: 'by role=assistant', filter: { role: 'assistant' } },
        {
          label: 'contextType as string',
          filter: { contextType: 'conversation' },
        },
        {
          label: 'contextType as enum value',
        ...
        }, // Last hour
        {
          ...
          filter: { userId, contextType: ContextType.CONVERSATION },
        },
      ];
...
async function executeTestQuery(
  ...
  try {
    ...
    try {
      ...
      logger.info(
        `History check after delay: ${immediateHistory.length} turns retrieved`,
      );
      ...
      const conversationMatches =
        directQueryResult.matches?.filter(
          (match) => match.metadata?.conversationId === conversationId,
        ) || [];

      logger.info(
        `Direct query after delay: ${conversationMatches.length}/${directQueryResult.matches?.length || 0} matches`,
        {
          firstMatch:
            conversationMatches.length > 0
              ? JSON.stringify(conversationMatches[0].metadata).substring(
                  0,
                  100,
                ) + '...'
              : 'No matches',
      ...
      logger.info(
        `Turn ID query after delay: ${turnIdQueryResult.matches?.length || 0} matches`,
        {
          turnId: assistantTurnId,
          result:
            turnIdQueryResult.matches && turnIdQueryResult.matches.length > 0
              ? 'Found'
              : 'Not found',
        },
      );
...
class TestAssistantAgent extends BaseAgent {
  ...
  async executeInternal(request: AgentRequest): Promise<AgentResponse> {
    ...
    const input =
      typeof request.input === 'string' ? request.input : 'complex input';
