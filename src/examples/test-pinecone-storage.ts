...
async function testPineconeStorage(): Promise<void> {
  ...
  const testMessage = 'This is a test message for Pinecone storage verification';
  ...
  try {
    ...
    logger.info('Vector stored successfully, waiting 30 seconds for indexing...');
    ...
    logger.info('Index stats after storage', {
      ...
      hasOurNamespace: (stats.namespaces && stats.namespaces[userId]) ? true : false,
      ourNamespaceStats: stats.namespaces && stats.namespaces[userId] ? stats.namespaces[userId] : 'not found'
    });
    ...
    const directIndex = pineconeIndexService.getIndex('user-context').namespace(userId);
    try {
    ...
    const filterVariations = [
      ...
      { label: 'by contextType', filter: { contextType: ContextType.CONVERSATION } },
      { label: 'by role', filter: { role: 'user' } },
      { label: 'by multiple filters', filter: { contextType: ContextType.CONVERSATION, role: 'user' } },
      { label: 'by timestamp range', filter: { timestamp: { $gte: Date.now() - 3600000 } } },
    ...
    try {
      ...
      logger.info(`  Results in entire index: ${result.matches?.length || 0} matches`);
      
      // Check if our vectors are in the results
      const foundOurRecord = result.matches?.some(match => match.id === recordId);
      logger.info(`  Found our record in results: ${foundOurRecord}`);
      
      // Show all namespaces found in results
      const namespacesInResults = new Set(result.matches?.map(match => match.metadata?.['namespace'] || 'unknown') || []);
      logger.info(`  Namespaces found in results: ${Array.from(namespacesInResults).join(', ')}`);
    } catch (error) {
