...
describe('jira-ticket-generator', () => {
  ...
  describe('extractValidObjects', () => {
    it('should extract valid JSON objects from a string', () => {
      const input =
        'Text before {"valid": true} text between {"also": "valid"} text after';
    ...
    it('should ignore invalid JSON objects', () => {
      const input =
        '{"valid": true} {"invalid": missing quotes} {"valid": "again"}';
  ...
  describe('cleanJsonArray', () => {
    ...
    it('should handle complex malformed JSON by extracting valid objects', () => {
      ...
      expect(result.some((item) => (item as any).ticketType === 'Bug')).toBe(
        true,
      );
      expect(
        result.some((item) => (item as any).ticketType === 'Feature'),
      ).toBe(true);
      expect(result.some((item) => (item as any).ticketType === 'Story')).toBe(
        true,
      );

      // Verify we don't have the invalid object
      expect(result.some((item) => (item as any).ticketType === 'Task')).toBe(
        false,
      );
    });
  ...
  describe('generateJiraTickets', () => {
    it('should process transcript chunks and return valid tickets', async () => {
      ...
      (splitTranscriptModule.splitTranscript as jest.Mock).mockReturnValue(
        mockChunks,
      );
      ...
      (processChunkModule.processAllChunks as jest.Mock).mockResolvedValue(
        mockPartialTickets,
      );
      ...
      expect(splitTranscriptModule.splitTranscript).toHaveBeenCalledWith(
        transcript,
        1600,
        4,
      );
    ...
    it('should handle errors and rethrow them', async () => {
      ...
      (processChunkModule.processAllChunks as jest.Mock).mockRejectedValue(
      ...
      // Execute and assert
      await expect(generateJiraTickets(transcript)).rejects.toThrow(
        errorMessage,
      );
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        'Error in generateJiraTickets:',
        expect.any(Error),
      );
