  ...
  private createProcessChunkNode() {
    return async (state: MeetingAnalysisState) => {
      try {
        ...
        // Process the current chunk using the agent
        const result = await this.agent.execute({
          input: `Process this chunk of a meeting transcript and extract key information:\n\n${chunk}`,
  ...
  private createGenerateFinalAnalysisNode() {
    return async (state: MeetingAnalysisState) => {
      try {
        ...
        const result = await this.agent.execute({
          input: `Generate a final comprehensive analysis based on these partial analyses of a meeting transcript:\n\n${combinedAnalyses}`,
