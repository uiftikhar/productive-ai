...
async function supervisorWithFallbackExample() {
  ...
  try {
    ...
    if (defaultAgent) {
      logger.info(`Default agent configured: ${defaultAgent.name} (${defaultAgent.id})`);
    } else {
      logger.warn('No default agent configured. Fallback functionality will be limited.');
    }
    ...
    logger.info(`Available agents: ${agentRegistry.listAgents().map(a => a.name).join(', ')}`);
