...
export class PerformanceMonitor {
  ...
  private constructor(logger?: Logger) {
    ...
    this.metricsInterval = setInterval(() => this.captureSystemMetrics(), 30000).unref();
  ...
  public apiMonitoringMiddleware() {
    return (req: Request, res: Response, next: NextFunction) => {
      ...
      res.end = function(this: Response, chunk?: any, encoding?: BufferEncoding, callback?: () => void) {
        ...
        PerformanceMonitor.instance.metrics.endpointMetrics[endpoint].averageResponseTime = 
          (PerformanceMonitor.instance.metrics.endpointMetrics[endpoint].averageResponseTime * 
            (PerformanceMonitor.instance.metrics.endpointMetrics[endpoint].count - 1) + 
            responseTime) / 
          PerformanceMonitor.instance.metrics.endpointMetrics[endpoint].count;
        
        // Track status code
        const statusCode = res.statusCode.toString();
        if (!PerformanceMonitor.instance.metrics.statusCodeDistribution[statusCode]) {
          PerformanceMonitor.instance.metrics.statusCodeDistribution[statusCode] = 0;
        }
        PerformanceMonitor.instance.metrics.statusCodeDistribution[statusCode]++;
        
        // Track errors
        if (res.statusCode >= 400) {
          PerformanceMonitor.instance.trackError();
          PerformanceMonitor.instance.metrics.endpointMetrics[endpoint].errors++;
        }
        
        // Add response time header for debugging
        res.setHeader('X-Response-Time', `${responseTime}ms`);
        // Call original end
        return originalEnd.apply(this, [chunk, encoding as BufferEncoding, callback]);
      } as any;
  ...
  public socketMonitoringMiddleware() {
    return (socket: Socket, next: (err?: Error) => void) => {
      ...
      socket.emit = function(this: Socket, ev: string, ...args: any[]): boolean {
  ...
  public trackSocketMessageSent() {
    ...
    this.lastMinuteWsMessagesSent = this.lastMinuteWsMessagesSent.filter(time => now - time < 60000);
  ...
  public trackSocketMessageReceived() {
    ...
    // Remove messages older than 1 minute
    this.lastMinuteWsMessagesReceived = this.lastMinuteWsMessagesReceived.filter(time => now - time < 60000);
    this.metrics.messagesReceivedPerMinute = this.lastMinuteWsMessagesReceived.length;
  }
