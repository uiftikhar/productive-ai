...
import { UserContextService, ContextType } from '../../shared/user-context/user-context.service.ts';
...
export class RagPromptManager {
   ...
   */
  private async retrieveUserContext(options: RagContextOptions): Promise<RetrievedContext> {
    const {
      userId,
      queryEmbedding,
    ...
    switch (strategy) {
      case RagRetrievalStrategy.SEMANTIC:
        ...
        const semanticResults = await this.userContextService.retrieveRagContext(
          userId,
          queryEmbedding,
          {
            topK: maxItems,
      ...
      case RagRetrievalStrategy.HYBRID:
        ...
        const semanticMatches = await this.userContextService.retrieveRagContext(
          userId,
          queryEmbedding,
          {
        ...
        if (conversationId) {
          conversationMatches = await this.userContextService.getConversationHistory(
            userId,
            conversationId,
      ...
      case RagRetrievalStrategy.CONVERSATION:
        ...
        if (!conversationId) {
          throw new Error('conversationId is required for CONVERSATION strategy');
        }
      ...
      case RagRetrievalStrategy.DOCUMENT:
        ...
        for (const docId of documentIds) {
          const chunks = await this.userContextService.getDocumentChunks(userId, docId);
          contextItems.push(...chunks);
        }
        
        // Sort by chunk index to maintain document order
        contextItems = contextItems.sort((a: any, b: any) => 
      ...
      case RagRetrievalStrategy.RECENCY:
        ...
        contextItems = recentResults
          .sort((a: any, b: any) => (b.metadata?.timestamp || 0) - (a.metadata?.timestamp || 0))
          .slice(0, maxItems);
        break;
   ...
   */
  private formatContextForPrompt(contextItems: Array<{
    content: string;
    source?: string;
    score?: number;
  }>): string {
