...
class MockLogger implements Logger {
  public messages: { level: LogLevel; message: string; context?: Record<string, any> }[] = [];
...
describe('EmbeddingAdapter', () => {
  ...
  beforeEach(() => {
    ...
    (EmbeddingServiceFactory.getService as jest.Mock).mockImplementation((options = {}) => {
  ...
  describe('constructor', () => {
    test('should initialize with default options', () => {
      ...
      expect(EmbeddingServiceFactory.getService).toHaveBeenCalledWith(expect.objectContaining({
  ...
  describe('generateEmbedding', () => {
    ...
    test('should fall back to embedText when generateEmbedding not available', async () => {
      ...
      expect(result).toEqual(sampleEmbedding);
      expect(mockLogger.hasMessage('Falling back to embedText method', 'debug')).toBe(true);
    });
    ...
    test('should fall back to embedQuery when other methods not available', async () => {
      ...
      expect(result).toEqual(sampleEmbedding);
      expect(mockLogger.hasMessage('Falling back to embedQuery method', 'debug')).toBe(true);
    });
    ...
    test('should return zero vector as last resort', async () => {
      ...
      expect(mockLogger.hasMessage('No embedding method found', 'warn')).toBe(true);
    });
  ...
  describe('calculateCosineSimilarity', () => {
    test('should call underlying service method when available', () => {
      ...
      // Assert
      expect(mockEmbeddingService.calculateCosineSimilarity).toHaveBeenCalledWith(vec1, vec2);
    ...
    test('should implement similarity locally when method not available', () => {
      ...
      expect(mockLogger.hasMessage('Implementing cosine similarity locally', 'debug')).toBe(true);
    });
    ...
    test('should throw error for mismatched dimensions', () => {
      ...
      // Act & Assert
      expect(() => adapter.calculateCosineSimilarity(vec1, vec2))
  ...
  describe('findSimilarEmbeddings', () => {
    test('should call underlying service method when available', () => {
      ...
      // Act
      const result = adapter.findSimilarEmbeddings(queryEmbedding, embeddingsWithMetadata);
      
      // Assert
      expect(mockEmbeddingService.findSimilarEmbeddings).toHaveBeenCalledWith(
        queryEmbedding, embeddingsWithMetadata, 5
      );
    ...
    test('should implement locally when method not available and use limit', () => {
      ...
      // Act
      const result = adapter.findSimilarEmbeddings(queryEmbedding, embeddingsWithMetadata, limit);
      
      // Assert
      expect(result).toHaveLength(1);
      expect(result[0].similarity).toBe(0.9);
      expect(result[0].metadata.id).toBe(1);
      expect(mockLogger.hasMessage('Implementing findSimilarEmbeddings locally', 'debug')).toBe(true);
    });
  });
  ...
  describe('combineEmbeddings', () => {
    test('should call underlying service method when available', () => {
      ...
      // Assert
      expect(mockEmbeddingService.combineEmbeddings).toHaveBeenCalledWith(embeddings);
    ...
    test('should implement locally when method not available', () => {
      ...
      expect(mockLogger.hasMessage('Implementing combineEmbeddings locally', 'debug')).toBe(true);
    ...
    test('should throw error when no embeddings provided', () => {
      ...
      // Act & Assert
      expect(() => adapter.combineEmbeddings([]))
    ...
    test('should throw error for embeddings with different dimensions', () => {
      ...
      // Act & Assert
      expect(() => adapter.combineEmbeddings(embeddings))
  ...
  describe('alternative interface methods', () => {
    test('embedText should delegate to generateEmbedding', async () => {
      ...
      const generateEmbeddingSpy = jest.spyOn(adapter, 'generateEmbedding')
      ...
      expect(result).toBe(sampleEmbedding);
      expect(mockLogger.hasMessage('Using embedText (alternative interface)', 'debug')).toBe(true);
    });
    ...
    test('embedBatch should call generateEmbedding for each text', async () => {
      ...
      expect(mockLogger.hasMessage('Using embedBatch (alternative interface)', 'debug')).toBe(true);
    });
