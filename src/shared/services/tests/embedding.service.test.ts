...
describe('EmbeddingService', () => {
  ...
  const sampleShortEmbedding = Array(3072)
    .fill(0)
    .map((_, i) => i / 10000);
  ...
  beforeEach(() => {
    ...
    mockOpenAIConnector.generateEmbedding = jest
      .fn()
      .mockResolvedValue(sampleShortEmbedding);
  ...
  describe('generateEmbedding', () => {
    test('should generate embedding for short text directly', async () => {
      ...
      expect(mockOpenAIConnector.generateEmbedding).toHaveBeenCalledWith(
        shortText.trim(),
      );
    ...
    test('should use chunking for long text', async () => {
      ...
      expect(
        mockOpenAIConnector.generateEmbedding.mock.calls.length,
      ).toBeGreaterThan(1);
  ...
  describe('combineEmbeddings', () => {
    test('should combine multiple embeddings into a normalized vector', () => {
      ...
      const magnitude = Math.sqrt(
        result.reduce((sum: number, val: number) => sum + val * val, 0),
      );
