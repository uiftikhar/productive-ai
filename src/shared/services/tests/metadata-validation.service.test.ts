import { MetadataValidationService } from '../user-context/metadata-validation.service';
import { UserContextValidationError } from '../user-context/types/context.types';
import { MemoryType } from '../user-context/types/memory.types';
import { TemporalRelevanceModel } from '../user-context/types/temporal.types';
...
describe('MetadataValidationService', () => {
  ...
  describe('validateMetadata', () => {
    test('should pass when all required fields are present', () => {
      ...
      // Act & Assert
      expect(() =>
        service.validateMetadata(metadata, requiredFields),
      ).not.toThrow();
    });
    ...
    test('should throw when a required field is missing', () => {
      ...
      // Act & Assert
      expect(() => service.validateMetadata(metadata, requiredFields)).toThrow(
        UserContextValidationError,
      );
      expect(() => service.validateMetadata(metadata, requiredFields)).toThrow(
        'Required metadata field "field2" is missing',
      );
    });
  ...
  describe('validateThemeMetadata', () => {
    ...
    test('should throw when theme arrays have different lengths', () => {
      ...
      // Act & Assert
      expect(() => service.validateThemeMetadata(metadata)).toThrow(
        UserContextValidationError,
      );
      expect(() => service.validateThemeMetadata(metadata)).toThrow(
    ...
    test('should throw when theme relevance score is outside valid range', () => {
      ...
      expect(() => service.validateThemeMetadata(metadata)).toThrow(
        'Theme relevance score must be between 0 and 1',
      );
    });
  ...
  describe('validateRoleMetadata', () => {
    ...
    test('should throw when role relevance score is outside valid range', () => {
      ...
      // Act & Assert
      expect(() => service.validateRoleMetadata(metadata)).toThrow(
        UserContextValidationError,
      );
      expect(() => service.validateRoleMetadata(metadata)).toThrow(
        'Role relevance score must be between 0 and 1',
      );
    });
  ...
  describe('validateMemoryMetadata', () => {
    ...
    test('should throw with invalid memory type', () => {
      ...
      // Act & Assert
      expect(() => service.validateMemoryMetadata(metadata)).toThrow(
        UserContextValidationError,
      );
      expect(() => service.validateMemoryMetadata(metadata)).toThrow(
        'Invalid memory type',
  ...
  describe('validateTemporalMetadata', () => {
    ...
    test('should throw with invalid temporal relevance model', () => {
      ...
      // Act & Assert
      expect(() => service.validateTemporalMetadata(metadata)).toThrow(
        UserContextValidationError,
      );
      expect(() => service.validateTemporalMetadata(metadata)).toThrow(
        'Invalid temporal relevance model',
      );
    });
  ...
  describe('validateAllMetadata', () => {
    test('should validate all types of metadata', () => {
      ...
      const validateThemeMetadataSpy = jest.spyOn(
        service,
        'validateThemeMetadata',
      );
      const validateRoleMetadataSpy = jest.spyOn(
        service,
        'validateRoleMetadata',
      );
      const validateMemoryMetadataSpy = jest.spyOn(
        service,
        'validateMemoryMetadata',
      );
      const validateTemporalMetadataSpy = jest.spyOn(
        service,
      ...
      // Assert
      expect(validateMetadataSpy).toHaveBeenCalledWith(metadata, [
        'requiredField',
      ]);
    ...
    test('should handle a metadata object that fails multiple validations', () => {
      ...
      expect(() => service.validateAllMetadata(metadata)).toThrow(
        UserContextValidationError,
      );
    });
  });
