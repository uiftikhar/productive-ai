...
describe('RagPromptManager Context Retrieval Strategies', () => {
  ...
  beforeEach(() => {
    ...
    ragPromptManager['retrieveUserContext'] = jest
      .fn()
      .mockImplementation((options: RagContextOptions) => {
        ...
        if (
          ...
          return Promise.reject(
            new Error('conversationId is required for CONVERSATION strategy'),
          );
        }
        ...
        if (
          ...
          return Promise.reject(
            new Error('documentIds are required for DOCUMENT strategy'),
          );
        }
        ...
        switch (strategy) {
          case RagRetrievalStrategy.SEMANTIC:
            items = documentContextItems.filter(
              (item) => item.score >= (options.minRelevanceScore || 0),
            );
          ...
          case RagRetrievalStrategy.HYBRID:
            items = [
              ...
              ...(options.conversationId ? conversationContextItems : []),
            ];
            sources.push('intro-to-ml.pdf');
    ...
    ragPromptManager['buildContextFilter'] = jest
      .fn()
      .mockImplementation((contentTypes, timeRangeStart, documentIds) => {
  ...
  describe('CONVERSATION strategy', () => {
    ...
    test('should throw error if conversationId is missing', async () => {
      ...
      await expect(
        ragPromptManager['retrieveUserContext'](options),
  ...
  describe('HYBRID strategy', () => {
    test('should combine semantic search with conversation context', async () => {
      ...
      expect(result.sources).toEqual(
        expect.arrayContaining(['intro-to-ml.pdf', 'conv1']),
      );
    });
  });
  ...
  describe('Context filter building', () => {
    ...
    test('should build filter with time range', () => {
      ...
      const filter = ragPromptManager['buildContextFilter'](
        undefined,
        timeRangeStart,
      );
