...
async function ragExample() {
  ...
  const userEmbedding1 = Array(1536).fill(0).map(() => Math.random() * 2 - 1);
  const assistantEmbedding1 = Array(1536).fill(0).map(() => Math.random() * 2 - 1);
  const userEmbedding2 = Array(1536).fill(0).map(() => Math.random() * 2 - 1);
  ...
  const documentChunks = [
    {
      content: 'TensorFlow is an open-source machine learning framework developed by Google. It has a comprehensive ecosystem of tools and libraries that makes it suitable for a wide range of ML tasks.',
      embedding: Array(1536).fill(0).map(() => Math.random() * 2 - 1)
    },
    {
      content: 'PyTorch is a machine learning framework developed by Facebook. It is known for its ease of use and dynamic computational graph, making it popular among researchers.',
      embedding: Array(1536).fill(0).map(() => Math.random() * 2 - 1)
    },
    {
      content: 'scikit-learn is a simple and efficient tool for data mining and data analysis. It is built on NumPy, SciPy, and matplotlib, making it accessible for beginners.',
      embedding: Array(1536).fill(0).map(() => Math.random() * 2 - 1)
    },
    {
      content: 'For beginners, scikit-learn offers the gentlest learning curve with its consistent API and extensive documentation. PyTorch is also beginner-friendly, especially for those interested in deep learning.',
      embedding: Array(1536).fill(0).map(() => Math.random() * 2 - 1)
    }
  ];
  ...
  // New user query
  const userQuery = 'Which ML framework has the best documentation for beginners?';
  ...
  const contextForRAG = relevantContext.map((item: any) => {
    ...
    return {
      content: item.metadata?.contextType === ContextType.CONVERSATION 
