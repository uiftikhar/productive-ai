...
export class KnowledgeGapService extends BaseContextService {
  ...
  async detectUnansweredQuestionGaps(
    ...
    return result.matches.map(match => {
      ...
      const questionAge = (Date.now() - (metadata.timestamp as number || 0)) / (24 * 60 * 60 * 1000);
      ...
      if (Array.isArray(metadata.participantIds) && metadata.participantIds.length > 3) {
        confidence += 0.05;
      }
      ...
      const contentStr = typeof metadata.content === 'string' ? metadata.content : '';
  ...
  async detectTeamMisalignments(
    ...
    if (!teamIds || teamIds.length < 2) {
      throw new Error('At least two team IDs are needed to detect misalignments');
    }
    ...
    const teamTopics: Record<string, {
      id: string;
      topicId: string;
      topicName: string;
      embedding: number[];
  ...
  async detectMissingInformation(
    ...
    for (const match of result.matches) {
      ...
      if (metadata.contextType === ContextType.QUESTION) {
        ...
        confidence = 0.75;
        const contentStr = typeof metadata.content === 'string' ? metadata.content : '';
        const partialAnswerStr = typeof metadata.partialAnswer === 'string' ? metadata.partialAnswer : '';
        ...
        const completeness = typeof metadata.completeness === 'number' ? metadata.completeness : 0;
        confidence = 1 - completeness;
        const topicNameStr = typeof metadata.topicName === 'string' ? metadata.topicName : '';
  ...
  async updateKnowledgeGapStatus(
    ...
    // Get the current gap
    const result = await this.executeWithRetry(
      () => this.pineconeService.fetchVectors(
        USER_CONTEXT_INDEX,
        [gapId],
        userId,
      ),
  ...
  async getKnowledgeGaps(
    ...
    return result.matches.map(match => {
      ...
      return {
        ...
        status: metadata.gapStatus as 'open' | 'addressed' | 'closed' | 'rejected',
