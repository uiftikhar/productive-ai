...
describe('DocumentContextService', () => {
  ...
  beforeEach(() => {
    ...
    (BaseContextService.prototype as any).storeUserContext = jest.fn().mockResolvedValue('mock-doc-id-123');
  ...
  describe('storeDocumentChunk', () => {
    ...
    test('should throw error when document ID is missing', async () => {
      ...
      // Act & Assert
      await expect(service.storeDocumentChunk(
        userId,
        documentId,
        documentTitle,
        content,
  ...
  describe('searchDocumentContent', () => {
    test('should search with basic filter', async () => {
      ...
      // Act
      const result = await service.searchDocumentContent(userId, queryEmbedding);
