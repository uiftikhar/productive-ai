...
export class MockEmbeddingService implements IEmbeddingService {
  ...
  async generateEmbeddingWithMetadata(text: string, metadata?: Record<string, any>): Promise<EmbeddingWithMetadata> {
  ...
  async findSimilarContent(query: string, content: string[]): Promise<{text: string, similarity: number}[]> {
    const queryEmbedding = await this.generateEmbedding(query);
    
    const results = await Promise.all(content.map(async (text) => {
      const embedding = await this.generateEmbedding(text);
  ...
  async searchInLongText(query: string, longText: string, options?: { chunkSize?: number, overlap?: number }): Promise<{text: string, similarity: number}[]> {
    ...
    for (let i = 0; i < longText.length; i += chunkSize - overlap) {
      ...
      if (chunk.length > 10) { // Only include chunks with meaningful content
        chunks.push(chunk);
      }
    }
...
export const createMockEmbeddingService = (logger?: Logger): IEmbeddingService => {
