...
describe('UserContextService', () => {
  ...
  describe('Knowledge gap detection methods', () => {
    it('should detect missing information knowledge gaps', async () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith('Detected knowledge gaps', expect.any(Object));
    });
    ...
    it('should detect misalignment knowledge gaps', async () => {
      ...
      jest.spyOn(userContextService as any, 'calculateCosineSimilarity').mockReturnValue(0.3);
    ...
    it('should return empty array when no gaps detected', async () => {
      ...
      jest.spyOn(userContextService as any, 'calculateCosineSimilarity').mockReturnValue(0.9);
    ...
    it('should validate inputs correctly', async () => {
      ...
      await expect(userContextService.detectKnowledgeGaps(
        'user1',
        ['team1'], // Only one team
        [[0.1, 0.2, 0.3]],
        ['Topic']
    ...
    it('should handle errors during knowledge gap detection', async () => {
      mockPineconeService.queryVectors = jest.fn().mockRejectedValue(new Error('Query failed'));
  ...
  describe('Pre-meeting context generation', () => {
    it('should generate pre-meeting context', async () => {
      ...
      mockPineconeService.queryVectors = jest.fn()
        ...
        .mockImplementationOnce(() => Promise.resolve({
          matches: [{ id: 'decision1', metadata: { decisionSummary: 'Decision 1' } }]
        }))
        // Action items
        .mockImplementationOnce(() => Promise.resolve({
          matches: [{ id: 'action1', metadata: { actionItemId: 'a1', status: ActionItemStatus.PENDING } }]
        }))
    ...
    it('should validate inputs for pre-meeting context', async () => {
      ...
      await expect(userContextService.generatePreMeetingContext(
        'user1',
        '', // Empty meeting ID
        ['user1'],
        [{ agendaItemId: 'a1', title: 'Title', description: 'Desc', embeddings: [0.1] }]
      )).rejects.toThrow(UserContextValidationError);
  ...
  describe('External system integration', () => {
    it('should integrate action items with external systems', async () => {
      ...
      const result = await userContextService.integrateActionItemWithExternalSystem(
        'user1',
        'a1',
        'jira',
        'JIRA-123',
    ...
    it('should validate inputs for external system integration', async () => {
      await expect(userContextService.integrateActionItemWithExternalSystem(
        'user1',
  ...
  describe('Advanced context retrieval with pagination', () => {
    it('should retrieve context with pagination', async () => {
      ...
      mockPineconeService.queryVectors = jest.fn()
        .mockImplementationOnce(() => Promise.resolve({
          matches: Array(25).fill(0).map((_, i) => ({ id: `item-${i}` }))
        }))
        // Mock the actual retrieval query
        .mockImplementationOnce(() => Promise.resolve({
          matches: Array(25).fill(0).map((_, i) => ({
            id: `item-${i}`,
            score: 0.9 - (i * 0.01),
            metadata: {
  ...
  describe('Usage tracking methods', () => {
    ...
    it('should handle errors gracefully when recording access', async () => {
      mockPineconeService.fetchVectors = jest.fn().mockRejectedValue(new Error('Fetch failed'));
    ...
    it('should provide relevance feedback', async () => {
      ...
      await userContextService.provideRelevanceFeedback('user1', 'context1', 0.8);
    ...
    it('should validate relevance feedback value', async () => {
      await expect(userContextService.provideRelevanceFeedback(
        'user1',
        'context1',
        1.5 // Out of range
      )).rejects.toThrow(UserContextValidationError);
  ...
  describe('Helper methods', () => {
    it('should calculate cosine similarity correctly', () => {
      ...
      const similarity1_2 = (userContextService as any).calculateCosineSimilarity(vec1, vec2);
      const similarity1_3 = (userContextService as any).calculateCosineSimilarity(vec1, vec3);
    ...
    it('should remove duplicates by property', () => {
      ...
      const uniqueItems = (userContextService as any).removeDuplicatesByProperty(items, 'id');
      
      expect(uniqueItems).toHaveLength(3);
      expect(uniqueItems.map((item: any) => item.id).sort()).toEqual(['a', 'b', 'c']);
